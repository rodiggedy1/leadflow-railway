...
CREATE TABLE `chain_executions` (
	...
	`plan` json NOT NULL,
	`status` enum('planned','awaiting_confirmation','running','succeeded','partial','failed','cancelled') NOT NULL DEFAULT 'planned',
	`createdAt` datetime(3) NOT NULL,
	`confirmedAt` datetime(3),
	`completedAt` datetime(3),
	CONSTRAINT `chain_executions_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `chain_step_executions` (
	`id` varchar(64) NOT NULL,
	`chainExecutionId` varchar(64) NOT NULL,
	`stepId` varchar(128) NOT NULL,
	`capabilityId` varchar(128) NOT NULL,
	`status` enum('planned','running','succeeded','failed','skipped','cancelled') NOT NULL DEFAULT 'planned',
	`resolvedArgs` json,
	`result` json,
	`verificationResult` json,
	`idempotencyKey` varchar(128),
	`externalResultId` varchar(255),
	`errorMessage` text,
	`createdAt` datetime(3) NOT NULL,
	`startedAt` datetime(3),
	`completedAt` datetime(3),
	CONSTRAINT `chain_step_executions_id` PRIMARY KEY(`id`),
	CONSTRAINT `chain_step_executions_idempotencyKey_unique` UNIQUE(`idempotencyKey`),
	CONSTRAINT `uq_chain_step` UNIQUE(`chainExecutionId`,`stepId`)
);
--> statement-breakpoint
...
CREATE TABLE `cleaner_jobs` (
	...
	`hasStripeCard` tinyint NOT NULL DEFAULT 0,
	`stripeCustomerId` varchar(255),
	`paymentBrand` varchar(32),
	`paymentLast4` varchar(4),
	`chargesOnHoldCents` int NOT NULL DEFAULT 0,
	`chargesOutstandingCents` int NOT NULL DEFAULT 0,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `conversation_sessions` (
	...
	`csResolvedAt` bigint,
	`csResolvedBy` varchar(128),
	`csResolvedByName` varchar(255),
	`csQueue` varchar(32),
...
CREATE TABLE `cs_missions` (
	...
	`title` varchar(255) NOT NULL,
	`emoji` varchar(16),
	`status` enum('active','waiting','ready','sending','completed','cancelled','needs_attention') NOT NULL DEFAULT 'active',
	`stages` json NOT NULL,
	`sortOrder` int NOT NULL DEFAULT 0,
	`createdAt` datetime(3) NOT NULL,
	`updatedAt` datetime(3) NOT NULL,
	`completedAt` datetime(3),
	`missionType` varchar(32) NOT NULL DEFAULT 'MANUAL',
	`jobId` bigint,
	`cleanerPhone` varchar(32),
	`cleanerName` varchar(160),
	`customerPhone` varchar(32),
	`customerName` varchar(160),
	`activeDedupKey` varchar(128),
	`failureReason` varchar(255),
	CONSTRAINT `cs_missions_id` PRIMARY KEY(`id`),
	CONSTRAINT `uq_cs_mission_active_dedup` UNIQUE(`activeDedupKey`)
);
--> statement-breakpoint
...
CREATE TABLE `focus_points` (
	...
	CONSTRAINT `uq_focus_points_agent_week` UNIQUE(`agentName`,`weekStart`)
);
--> statement-breakpoint
...
CREATE TABLE `invoiceTemplates` (
	...
	`stripeLink` varchar(1000) NOT NULL DEFAULT '',
...
--> statement-breakpoint
CREATE TABLE `madison_action_items` (
	...
	`createdAt` datetime(3) NOT NULL,
	CONSTRAINT `madison_action_items_id` PRIMARY KEY(`id`),
	CONSTRAINT `uq_action_item` UNIQUE(`actionId`,`readinessItemId`)
);
--> statement-breakpoint
CREATE TABLE `madison_actions` (
	`id` varchar(64) NOT NULL,
	...
	CONSTRAINT `madison_actions_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `madison_conversation_context` (
	`id` int AUTO_INCREMENT NOT NULL,
	`agentId` int NOT NULL,
	...
	`updatedAt` datetime(3) NOT NULL,
	CONSTRAINT `madison_conversation_context_id` PRIMARY KEY(`id`),
	CONSTRAINT `madison_conversation_context_agentId_unique` UNIQUE(`agentId`)
);
--> statement-breakpoint
CREATE TABLE `madison_email_drafts` (
	`id` bigint AUTO_INCREMENT NOT NULL,
	`threadId` varchar(255) NOT NULL,
	...
	CONSTRAINT `uq_email_draft_inbound` UNIQUE(`inboundMessageId`)
);
--> statement-breakpoint
CREATE TABLE `madison_missions` (
	`id` int AUTO_INCREMENT NOT NULL,
...
--> statement-breakpoint
CREATE TABLE `madison_sms_drafts` (
	`id` bigint AUTO_INCREMENT NOT NULL,
	...
	CONSTRAINT `uq_sms_draft_inbound` UNIQUE(`inboundOpenPhoneId`)
);
--> statement-breakpoint
...
CREATE TABLE `ops_chat_messages` (
	...
	`cardStatus` varchar(32) DEFAULT 'active',
	`activeDedupKey` varchar(128),
	CONSTRAINT `ops_chat_messages_id` PRIMARY KEY(`id`),
	CONSTRAINT `uq_ops_active_sms_card` UNIQUE(`activeDedupKey`)
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `readiness_acknowledgements` (
	`id` bigint AUTO_INCREMENT NOT NULL,
	...
	CONSTRAINT `readiness_acknowledgements_id` PRIMARY KEY(`id`),
	CONSTRAINT `uq_active_readiness_ack` UNIQUE(`jobId`,`serviceDate`,`issueType`,`activeMarker`)
);
--> statement-breakpoint
...
CREATE INDEX `idx_chain_exec_agent` ON `chain_executions` (`agentId`,`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_chain_step_chain` ON `chain_step_executions` (`chainExecutionId`,`stepId`);--> statement-breakpoint
...
CREATE INDEX `idx_cs_missions_session` ON `cs_missions` (`sessionId`);--> statement-breakpoint
CREATE INDEX `idx_cs_missions_session_status` ON `cs_missions` (`sessionId`,`status`);--> statement-breakpoint
...
CREATE INDEX `idx_focus_points_week` ON `focus_points` (`weekStart`);--> statement-breakpoint
...
CREATE INDEX `idx_invoices_templateId` ON `invoices` (`templateId`);--> statement-breakpoint
CREATE INDEX `idx_invoices_customerName` ON `invoices` (`customerName`);--> statement-breakpoint
...
CREATE INDEX `idx_action_item_ack` ON `madison_action_items` (`acknowledgementId`);--> statement-breakpoint
CREATE INDEX `idx_madison_actions_agent` ON `madison_actions` (`conversationAgentId`,`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_email_draft_status` ON `madison_email_drafts` (`status`);--> statement-breakpoint
CREATE INDEX `idx_email_draft_thread` ON `madison_email_drafts` (`threadId`);--> statement-breakpoint
CREATE INDEX `idx_email_draft_created` ON `madison_email_drafts` (`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_email_draft_status_created` ON `madison_email_drafts` (`status`,`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_madison_missions_agent_archived_created` ON `madison_missions` (`agentId`,`archivedAt`,`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_sms_draft_status` ON `madison_sms_drafts` (`status`);--> statement-breakpoint
CREATE INDEX `idx_sms_draft_session` ON `madison_sms_drafts` (`sessionId`);--> statement-breakpoint
CREATE INDEX `idx_sms_draft_capability` ON `madison_sms_drafts` (`capability`);--> statement-breakpoint
CREATE INDEX `idx_sms_draft_created` ON `madison_sms_drafts` (`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_sms_draft_status_created` ON `madison_sms_drafts` (`status`,`createdAt`);--> statement-breakpoint
CREATE INDEX `idx_sms_draft_outbound` ON `madison_sms_drafts` (`outboundOpenPhoneId`);--> statement-breakpoint
...
CREATE INDEX `idx_readiness_ack_lookup` ON `readiness_acknowledgements` (`jobId`,`serviceDate`,`issueType`,`reversedAt`);--> statement-breakpoint
CREATE INDEX `idx_readiness_ack_action` ON `readiness_acknowledgements` (`actionId`);--> statement-breakpoint
