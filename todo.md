...
- [x] End-of-day check-in: DB schema (teamAvailabilityCheckins table)
...
- [x] Add a staff-only Customer My Home test link at the existing Booking job-card magic-link location so it can be tested without a status SMS
...
- [x] Rebuild the live Customer Profile destination from the approved review shell without altering unrelated routes or submitting customer actions. Completed locally 2026-09-17: the approved shell now reads the existing selected conversation plus a read-only LeadFlow-owned service-record profile; it preserves the existing SMS handoff rather than creating a second sender. Focused source contract, scoped operational-data boundary, type-check review, browser rendering, build, and whitespace checks completed. Not deployed.
- [x] Add a LeadFlow-owned Customer Profile directory at `/admin/customer-profile`: the landing state now searches and lists existing LeadFlow-owned customer records, each row opens the existing detail shell, and CS Inbox deep links remain direct-detail entry points. Validated locally 2026-09-17 with a real authenticated list, name search, and detail open; no customer action was submitted. Not deployed.
- [x] Resolve a directory-opened customer to the existing canonical CS conversation by normalized phone, then feed that existing session ID into the unchanged merged conversation reader. Read-only only: no session creation, messages, calls, customer changes, or operational updates. Validated with the existing Daniel Foster conversation: 41 entries render from the directory-opened detail without the misleading warning.
