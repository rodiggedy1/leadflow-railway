        ...
        <section className="odr-primary-grid" aria-label="Live field operations overview">
          <article className="odr-card odr-jobs-card">
            ...
            <div className={`odr-map odr-map-${view}`}>
              {view === "map" && <div className="odr-live-schedule-map"><LeadflowScheduleMap jobs={(scheduleMapData?.jobs ?? []) as LeadflowScheduleMapJob[]} teams={(scheduleMapData?.teams ?? []) as LeadflowScheduleMapTeam[]} selectedJobId={active?.id ?? null} onJobSelect={selectMapJob} /></div>}
