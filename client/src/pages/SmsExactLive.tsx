...
function LiveCustomerPanel({ conversation, openTools, runMission }: { conversation: LiveConversation; openTools: () => void; runMission: (mission: CustomerMission) => void }) {
  ...
  return <aside className="cic-right-panel">
    ...
    <section className="cic-live-missions"><header><b>Missions</b><button type="button" onClick={openTools}>+ Add</button></header><div><button className="cic-live-mission" type="button" onClick={() => runMission("payment")}><i><CreditCard size={19} strokeWidth={1.8} /></i><span><b>Send Payment Link</b><small>Generate &amp; send a payment link via SMS.</small></span></button><button className="cic-live-mission" type="button" onClick={() => runMission("quote")}><i><FileText size={19} strokeWidth={1.8} /></i><span><b>Send Quote</b><small>Build &amp; send a personalized quote.</small></span></button><button className="cic-live-mission" type="button" onClick={() => runMission("agreement")}><i><FileText size={19} strokeWidth={1.8} /></i><span><b>Send Service Agreement</b><small>Send the Satisfaction Guarantee via SMS.</small></span></button></div></section>
...
function LiveToolDialog({ conversation, close, setCompose, messages, initialMission }: { conversation: LiveConversation; close: () => void; setCompose: (value: string) => void; messages: { sender: MsgSender; text: string; ts?: number }[]; initialMission: CustomerMission | null }) {
  return <div className="cic-live-tools-backdrop" role="dialog" aria-modal="true" aria-label="Conversation actions" onClick={close}><section className="cic-live-tools" onClick={event => event.stopPropagation()}><header><span>Conversation actions</span><button type="button" onClick={close}><X /></button></header><div className="cic-live-tools-content">{isTeamMember(conversation) ? <CsRightPanelTeam selected={{ id: conversation.id, name: conversation.name, initials: conversation.initials, phone: conversation.phone, queue: conversation.queue, status: conversation.csStatusTier ?? undefined, wait: conversation.wait }} /> : <CsRightPanelClient selected={{ id: conversation.id, name: conversation.name, initials: conversation.initials, phone: conversation.phone, queue: conversation.queue, status: conversation.csStatusTier ?? undefined, wait: conversation.wait, stats: { bookings: 0, complaints: 0 } }} setCompose={setCompose} messages={messages} initialMission={initialMission} />}</div></section></div>;
}
...
export default function SmsExactLive() {
  ...
  const [initialMission, setInitialMission] = useState<CustomerMission | null>(null);
  ...
  const selectConversation = (conversation: LiveConversation) => { window.dispatchEvent(new Event("review-workspace-collapse")); autoDraftedForRef.current = null; setSelected(conversation); setCompose(""); setComposeMode("reply"); setShowTools(false); setInitialMission(null); };
  const openTools = (mission: CustomerMission | null = null) => { setInitialMission(mission); setShowTools(true); };
