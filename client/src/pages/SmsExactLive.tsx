...
function LiveCustomerPanel({ conversation, openTools }: { conversation: LiveConversation; openTools: () => void }) {
  ...
  return <aside className="cic-right-panel">
    ...
    <section className="cic-live-missions"><header><b>Missions</b><button type="button" onClick={openTools}>+ Add</button></header><div>{[[CreditCard, "Send Payment Link", "Generate & send a payment link via SMS."], [FileText, "Send Quote", "Build & send a personalized quote."], [FileText, "Send Service Agreement", "Send the Satisfaction Guarantee via SMS."]].map(([Icon, label, copy]) => { const MissionIcon = Icon as typeof CreditCard; return <button className="cic-live-mission" key={label as string} type="button" onClick={openTools}><i><MissionIcon size={19} strokeWidth={1.8} /></i><span><b>{label as string}</b><small>{copy as string}</small></span></button>; })}</div></section>
...
export default function SmsExactLive() {
  ...
  const selectConversation = (conversation: LiveConversation) => { window.dispatchEvent(new Event("review-workspace-collapse")); autoDraftedForRef.current = null; setSelected(conversation); setCompose(""); setComposeMode("reply"); setShowTools(false); };
