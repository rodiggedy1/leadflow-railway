...
function invoiceState(invoice: InvoiceRecord): InvoiceState {
  if (invoice.paidAt || invoice.stripeInvoiceStatus === "paid") return "paid";
  // Native Stripe invoices use no stated due date in this approved flow, so they are never inferred overdue.
  if (invoice.stripeInvoiceId) return "open";
...
export default function InvoicesExactLive() {
  ...
  const sendInvoice = trpc.invoice.sendByEmail.useMutation({ onSuccess: (result) => { toast.success(`Invoice #${result.invoiceNumber} sent to ${result.toEmail}`); setShowShare(false); setShareEmail(""); void utils.invoice.listInvoices.invalidate(); }, onError: (invoiceError) => toast.error(invoiceError.message) });
  ...
  const selectedIsStripeManaged = Boolean(selected?.stripeInvoiceId);
