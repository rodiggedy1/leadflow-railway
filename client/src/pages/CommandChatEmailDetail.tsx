...
import "./command-chat-email-detail.css";
  ...
  return <main className="email-detail-main em2-main emails-csinbox2-detail-main">
    ...
    <section className="em2-thread">
      ...
      {detailError && !detail && <div className="emails-live-thread-error" role="alert">
        <strong>Couldn’t load the live Gmail thread.</strong>
        <p>{detailError}</p>
...
export default function EmailsExactLive({ initialThreadId = null, onCloseDetail, detailOnly = false }: EmailsExactLiveProps = {}) {
  ...
  const threadLoadError = emailThread.error
    ? emailThread.error.message || "The live Gmail request failed without a message."
    : null;
  const retryThreadLoad = () => { void emailThread.refetch(); };
