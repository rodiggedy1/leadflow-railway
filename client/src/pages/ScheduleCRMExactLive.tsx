...
import { ScheduleMap } from "@/components/SchedulingTab";
...
function ClientDrawer({ job, teams, onClose, onRaiseIssue }: { job: Job | null; teams: Team[]; onClose: () => void; onRaiseIssue: (job: Job) => void }) {
  ...
  const manualAssign = trpc.scheduling.manualAssign.useMutation({
    onSuccess: () => {
      if (job?.serviceDateTime) utils.scheduling.getSchedule.invalidate({ date: job.serviceDateTime.slice(0, 10) });
...
function TeamModal({ teams, selectedTeam, onClose, onChanged }: { teams: Team[]; selectedTeam: Team | null; onClose: () => void; onChanged: () => void }) {
  ...
  const upsert = trpc.scheduling.upsertTeam.useMutation({ onSuccess: () => { onChanged(); setName(""); setHomeAddress(""); setAdding(false); toast.success("Team added"); }, onError: error => toast.error(error.message) });
  const archive = trpc.scheduling.archiveTeam.useMutation({ onSuccess: onChanged, onError: error => toast.error(error.message) });
...
export default function ScheduleCRMExactLive() {
  const utils = trpc.useUtils();
  ...
  const { data, isLoading } = trpc.scheduling.getSchedule.useQuery({ date }, { staleTime: 30_000, refetchOnWindowFocus: false });
  const { data: jobLocks = [] } = trpc.scheduling.getJobLocks.useQuery({ date });
  const { data: analysisData, isFetching: analysisFetching } = trpc.scheduling.analyzeSchedule.useQuery({ date }, { enabled: showAnalysis, staleTime: 60_000 });
  const { data: dayIssues = [] } = trpc.calls.getDayIssues.useQuery({ jobDate: date }, { refetchInterval: 30_000 });
  const { data: suggestionData, isFetching: suggestionFetching } = trpc.scheduling.suggestSlots.useQuery({ address: suggestAddress, date }, { enabled: suggestAddress.length > 5, staleTime: 60_000 });
  useOpsStream({ onJobUpdate: () => utils.scheduling.getSchedule.invalidate({ date }) });

  const optimize = trpc.scheduling.optimizeDay.useMutation({ onSuccess: () => { utils.scheduling.getSchedule.invalidate({ date }); utils.scheduling.getJobLocks.invalidate({ date }); toast.success("Routes optimized"); }, onError: error => toast.error(error.message) });
  const reset = trpc.scheduling.resetOptimization.useMutation({ onSuccess: () => { utils.scheduling.getSchedule.invalidate({ date }); utils.scheduling.getJobLocks.invalidate({ date }); toast.success("Schedule reset — all assignments and locks cleared"); }, onError: error => toast.error(error.message) });
  const rerunDistances = trpc.scheduling.rerunDistances.useMutation({ onSuccess: result => { utils.scheduling.getSchedule.invalidate({ date }); toast.success(result.message); }, onError: error => toast.error(error.message) });
  const lockJob = trpc.scheduling.lockJob.useMutation({ onSuccess: () => utils.scheduling.getJobLocks.invalidate({ date }), onError: error => toast.error(error.message) });

  const jobs = (data?.jobs ?? []) as Job[];
  const teams = (data?.teams ?? []) as Team[];
  const lockedJobIds = useMemo(() => new Set(jobLocks.map(lock => lock.jobId)), [jobLocks]);
  ...
  return <main className="ocr-shell scr-shell schedule-crm-exact-live" id="schedule">
    ...
    <section className="scr-workspace">
      ...
      <section className="scr-schedule-main">{isLoading ? <div className="scr-live-loading">Loading live schedule…</div> : <div className="scr-route-stack">{teams.filter(team => !team.isArchived).map(team => <TeamRoute key={team.id} team={team} jobs={grouped.get(team.id) || []} selectedJobId={selectedJob?.id ?? null} lockedJobIds={lockedJobIds} conflictJobIds={conflictJobIds} onJobSelect={setSelectedJob} onTeamSelect={team => { setSelectedTeam(team); setShowTeams(true); }} />)}{(grouped.get(null) || []).length > 0 && <section className="scr-unassigned"><header><AlertCircle />Unassigned ({(grouped.get(null) || []).length})</header>{(grouped.get(null) || []).map(job => <LiveJobCard key={job.id} job={job} selected={selectedJob?.id === job.id} locked={lockedJobIds.has(job.id)} conflict={conflictJobIds.has(job.id)} onSelect={() => setSelectedJob(job)} />)}</section>}<section className="scr-weekly"><header><CalendarDays />Weekly Team Schedule <ChevronDown /></header><div>{teams.filter(team => team.isActive === 1 && !team.isArchived).map(team => <span key={team.id}>{team.name}<b>{team.workScheduleUnavailable ? "Off today" : "Available"}</b></span>)}</div></section></div>}
        <aside className="scr-map-panel"><header><span><Navigation />Route map</span><b>{currentTeamCount} teams</b></header><div className="scr-map-grid schedule-crm-live-map">{isLoading ? <div className="scr-live-map-loading">Loading route map…</div> : <ScheduleMap jobs={filteredJobs as never[]} teams={teams as never[]} selectedJobId={selectedJob?.id ?? null} onJobSelect={id => setSelectedJob(jobs.find(job => job.id === id) ?? null)} />}</div></aside></section>
    </section>
    {showTeams && <TeamModal teams={teams} selectedTeam={selectedTeam} onClose={() => { setShowTeams(false); setSelectedTeam(null); }} onChanged={() => { utils.scheduling.getTeams.invalidate(); utils.scheduling.getSchedule.invalidate({ date }); }} />}
    {showCalls && <CallLogPanel open={showCalls} onClose={() => setShowCalls(false)} jobDate={date} />}
    {issueJob && <IssueDialog open={Boolean(issueJob)} onClose={() => setIssueJob(null)} cleanerJobId={issueJob.id} jobDate={date} onCallFired={() => setShowCalls(true)} />}
