...
export default function CustomerPortal() {
  ...
  if (activePage === "home") {
    ...
    return <><CustomerPortalHome customerName={portal.data.account.name} homeAddress={homeAddress} nextBooking={nextCustomerBooking} nextBookingDate={nextCustomerBooking ? formatLocalDate(nextCustomerBooking.jobDate) : null} nextBookingDetails={detailParts.join(" · ") || null} bookingStatusLabel={nextCustomerBooking ? formatStatus(nextCustomerBooking.bookingStatus) : "No visit scheduled"} activePage={activePage} todayStatus={todayBookingWithLiveStatus ? <PortalTodayStatus job={todayBookingWithLiveStatus} booking={todayLeadflowBooking} savingNote={updateLeadflowJobCustomerNote.isPending} onUpdateNote={todayLeadflowBooking ? note => updateLeadflowJobCustomerNote.mutate({ id: todayLeadflowBooking.id, note }) : undefined} onViewBooking={openTodayBooking} /> : null} onGoToPage={goToPage} onBookHomeCleaning={() => setShowCleaningRebook(true)} onOpenService={openService} />{selectedService && <ServiceRequestForm service={selectedService} homeAddress={homeAddress} savedCard={portal.data.savedCard} customerName={portal.data.account.name} onClose={() => setSelectedService(null)} />}{showCleaningRebook && <div className="mib-portal-rebook-overlay" role="dialog" aria-modal="true" aria-label="Rebook home cleaning"><BookNow portalRebook={{ customerName: portal.data.account.name, phone: portal.data.account.phone, email: portal.data.account.email, address: homeAddress, savedCard: portal.data.savedCard }} onClose={closeCleaningRebook} /></div>}</>;
  }
  ...
  return <div className="mib-portal-shell mib-direct-portal" id="mib-home">
    ...
    <div className="mib-direct-layout">
      ...
      {records}</section>}
        ...
        {activePage === "messages" && <section className="mib-direct-page"><div className="mib-direct-page-title"><small>MESSAGES</small><h1>Messages</h1><p>Messages with your Maids in Black team, saved with the right booking.</p></div>{messageThreadsQuery.isLoading ? <div className="mib-direct-unavailable"><MessageSquare /><h2>Loading messages…</h2></div> : messageThreadsQuery.isError ? <div className="mib-direct-unavailable"><MessageSquare /><h2>Messages are temporarily unavailable.</h2><button className="mib-direct-button" type="button" onClick={() => messageThreadsQuery.refetch()}>Try again</button></div> : messageThreads.length === 0 ? <div className="mib-direct-unavailable"><MessageSquare /><h2>No messages yet.</h2><p>Messages from your cleaning team will appear here when they need to reach you about a booking.</p><button className="mib-direct-button" type="button" onClick={() => goToPage("bookings")}>View my bookings <ArrowRight /></button></div> : <div className="mib-portal-message-threads">{messageThreads.map(thread => <article key={thread.leadflowJobId} className="mib-portal-message-thread"><header><small>HOME CLEANING · {formatLocalDate(thread.jobDate)}</small><h2>{thread.serviceName || "Home cleaning"}</h2><p>{thread.jobAddress || "Address saved with booking"}</p></header><div className="mib-portal-message-bubbles">{thread.messages.map(message => <div key={message.id} className={message.senderRole === "customer" ? "is-customer" : "is-team"}><p>{message.body}</p><small>{message.senderRole === "customer" ? "You" : "Your cleaning team"}</small></div>)}</div><form onSubmit={event => { event.preventDefault(); const body = messageDrafts[thread.leadflowJobId]?.trim(); if (body) sendPortalReply.mutate({ leadflowJobId: thread.leadflowJobId, body }, { onSuccess: () => setMessageDrafts(current => ({ ...current, [thread.leadflowJobId]: "" })) }); }}><label htmlFor={`portal-message-${thread.leadflowJobId}`}>Reply to your cleaning team</label><textarea id={`portal-message-${thread.leadflowJobId}`} value={messageDrafts[thread.leadflowJobId] ?? ""} onChange={event => setMessageDrafts(current => ({ ...current, [thread.leadflowJobId]: event.target.value }))} maxLength={1_000} placeholder="Write a reply…" /><button className="mib-direct-button mib-direct-button-primary" type="submit" disabled={!messageDrafts[thread.leadflowJobId]?.trim() || sendPortalReply.isPending}>{sendPortalReply.isPending ? "Sending…" : "Send reply"}<ArrowRight /></button></form></article>)}</div>}</section>}
