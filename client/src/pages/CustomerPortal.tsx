...
export default function CustomerPortal() {
  ...
  const [focusedLeadflowJobId, setFocusedLeadflowJobId] = useState<number | null>(null);
  const [sameDayBookingFallback, setSameDayBookingFallback] = useState(false);
  ...
  const sameDayJob = todayJobStatus.data?.job ?? null;
  const sameDayLeadflowBooking = sameDayJob?.bookingId ? portal.data?.leadflowJobs.find(job => job.launch27BookingId === sameDayJob.bookingId) ?? null : null;
  useEffect(() => {
    if (activePage !== "bookings" || !focusedLeadflowJobId) return;
    const target = window.document.getElementById(`portal-leadflow-job-${focusedLeadflowJobId}`);
    target?.scrollIntoView({ behavior: "smooth", block: "center" });
  }, [activePage, focusedLeadflowJobId]);
  const startNewCardSetup = trpc.customerPortal.startNewCardSetup.useMutation();
  ...
  const openTodayBooking = () => {
    setFocusedLeadflowJobId(sameDayLeadflowBooking?.id ?? null);
    setSameDayBookingFallback(!sameDayLeadflowBooking);
    goToPage("bookings");
  };
  ...
  return <div className="mib-portal-shell mib-direct-portal" id="mib-home">
    ...
    <div className="mib-direct-layout">
      ...
      <main className="mib-direct-main">
        ...
        {sameDayJob ? <PortalTodayStatus job={sameDayJob} booking={sameDayLeadflowBooking} savingNote={updateLeadflowJobCustomerNote.isPending} onUpdateNote={sameDayLeadflowBooking ? note => updateLeadflowJobCustomerNote.mutate({ id: sameDayLeadflowBooking.id, note }) : undefined} onViewBooking={openTodayBooking} /> : <section className="mib-direct-stats" aria-label="Account summary">
      ...
      {sameDayBookingFallback && sameDayJob && <article className="mib-portal-booking-card mib-portal-booking-card-fallback"><div className="mib-portal-booking-top"><div><small>TODAY&apos;S BOOKING</small><h3>{sameDayJob.serviceType || "Home cleaning"}</h3><span className="mib-portal-status">{formatStatus(sameDayJob.bookingStatus || "scheduled")}</span></div></div><div className="mib-portal-booking-grid"><div><CalendarDays /><span>SCHEDULED APPOINTMENT</span><strong>{formatLocalDate(sameDayJob.jobDate)}</strong><p>{sameDayJob.serviceDateTime ? new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit" }).format(new Date(sameDayJob.serviceDateTime)) : "Time will be confirmed here."}</p></div><div><MapPin /><span>LOCATION</span><strong>{sameDayJob.jobAddress || "Address saved with booking"}</strong><p>{sameDayJob.teamName ? `${sameDayJob.teamName} is assigned to your service.` : "Your team assignment will appear here."}</p></div><div><ClipboardList /><span>UPDATES</span><strong>{sameDayJob.jobStatus ? formatStatus(sameDayJob.jobStatus) : "Scheduled"}</strong><p>{sameDayJob.customerNotes || "No notes added for today."}</p></div></div></article>}{records}</section>}
