...
import { CustomerPortalScheduleRequest, type CustomerPortalScheduleRequestBooking } from "@/components/CustomerPortalScheduleRequest";
import { CustomerPortalExtrasRequest } from "@/components/CustomerPortalExtrasRequest";
...
type PortalHomeBooking = CustomerPortalTodayJob & CustomerPortalScheduleRequestBooking & { focusTargetId: string; leadflowJobId: number | null };
...
function ServiceRequestForm({ service, homeAddress, savedCard, customerName, onClose }: { service: CustomerPortalService; homeAddress: string; savedCard: { brand: string | null; last4: string } | null; customerName: string; onClose: () => void }) {
  ...
  const isInteriorPainting = service.id === "interior-painting";
  const isMovingHelp = service.id === "moving-help";
  const isJunkRemoval = service.id === "junk-removal";
  const isPressureWashing = service.id === "pressure-washing";
  const usesBookingPanelTreatment = isLawnCare || isTvMounting || isFurnitureAssembly || isPictureHanging || isMinorHomeRepairs || isHandyman || isPlumbing || isElectricalLighting || isInteriorPainting || isMovingHelp || isJunkRemoval || isPressureWashing;
  const serviceCardFormId = isLawnCare ? "mib-lawn-care-card-form" : isTvMounting ? "mib-tv-mounting-card-form" : isFurnitureAssembly ? "mib-furniture-assembly-card-form" : isPictureHanging ? "mib-picture-hanging-card-form" : isMinorHomeRepairs ? "mib-minor-home-repairs-card-form" : isHandyman ? "mib-handyman-card-form" : isPlumbing ? "mib-plumbing-card-form" : isElectricalLighting ? "mib-electrical-lighting-card-form" : isInteriorPainting ? "mib-interior-painting-card-form" : isMovingHelp ? "mib-moving-help-card-form" : isJunkRemoval ? "mib-junk-removal-card-form" : "mib-pressure-washing-card-form";
  const serviceActionLabel = isLawnCare ? "lawn-care" : isTvMounting ? "TV-mounting" : isFurnitureAssembly ? "furniture-assembly" : isPictureHanging ? "picture-hanging" : isMinorHomeRepairs ? "minor-home-repairs" : isHandyman ? "handyman" : isPlumbing ? "plumbing" : isElectricalLighting ? "electrical & lighting" : isInteriorPainting ? "interior painting" : isMovingHelp ? "moving help" : isJunkRemoval ? "junk removal" : "pressure washing";
  const paymentMethodLabel = isFurnitureAssembly ? "Choose a payment method for Furniture assembly" : isPictureHanging ? "Choose a payment method for Picture hanging" : isMinorHomeRepairs ? "Choose a payment method for Minor home repairs" : isHandyman ? "Choose a payment method for Handyman visit" : isPlumbing ? "Choose a payment method for Plumbing help" : isElectricalLighting ? "Choose a payment method for Electrical & lighting" : isInteriorPainting ? "Choose a payment method for Interior painting" : isMovingHelp ? "Choose a payment method for Moving help" : isJunkRemoval ? "Choose a payment method for Junk removal" : isPressureWashing ? "Choose a payment method for Pressure washing" : "Payment method";
  const sendRequest = () => {
  ...
  return <div className={usesBookingPanelTreatment ? "mib-portal-rebook-overlay" : "mib-portal-modal"} role="dialog" aria-modal="true" aria-labelledby="mib-service-title">
    <div className={usesBookingPanelTreatment ? `mib-booking-panel mib-portal-rebook-panel mib-lawncare-request-panel${isTvMounting ? " mib-tv-mounting-request-panel" : ""}${isFurnitureAssembly ? " mib-furniture-assembly-request-panel" : ""}${isPictureHanging ? " mib-picture-hanging-request-panel" : ""}${isMinorHomeRepairs ? " mib-minor-home-repairs-request-panel" : ""}${isHandyman ? " mib-handyman-request-panel" : ""}${isPlumbing ? " mib-plumbing-request-panel" : ""}${isElectricalLighting ? " mib-electrical-lighting-request-panel" : ""}${isInteriorPainting ? " mib-interior-painting-request-panel" : ""}${isMovingHelp ? " mib-moving-help-request-panel" : ""}${isJunkRemoval ? " mib-junk-removal-request-panel" : ""}${isPressureWashing ? " mib-pressure-washing-request-panel" : ""}` : "mib-portal-modal-card"}>
      ...
      {!isLawnCare && !isFurnitureAssembly && !isPictureHanging && !isMinorHomeRepairs && !isHandyman && !isPlumbing && !isElectricalLighting && !isInteriorPainting && !isMovingHelp && !isJunkRemoval && !isPressureWashing && paymentChoice === "new" && !newCardSetup && <button type="button" className="mib-portal-secondary-action" disabled={startNewCardSetup.isPending} onClick={() => { void startNewCardSetup.mutateAsync().then(setNewCardSetup); }}>{startNewCardSetup.isPending ? "Preparing secure card entry…" : "Continue to secure card entry"}<ArrowRight /></button>}
...
export default function CustomerPortal() {
  ...
  const [scheduleRequestBooking, setScheduleRequestBooking] = useState<PortalHomeBooking | null>(null);
  const [extrasRequestBooking, setExtrasRequestBooking] = useState<PortalHomeBooking | null>(null);
  const utils = trpc.useUtils();
  ...
  const totalBookingCount = portal.data.cleanings.length + portal.data.leadflowJobs.length;
  ...
  if (activePage === "home") {
    ...
    return <><CustomerPortalHome customerName={portal.data.account.name} homeAddress={homeAddress} nextBooking={nextCustomerBooking} nextBookingDate={nextCustomerBooking ? formatLocalDate(nextCustomerBooking.jobDate) : null} nextBookingDetails={detailParts.join(" · ") || null} totalBookingCount={totalBookingCount} paymentMethodLabel={savedCardLabel} bookingStatusLabel={nextCustomerBooking ? formatStatus(nextCustomerBooking.bookingStatus) : "No visit scheduled"} activePage={activePage} todayStatus={todayBookingWithLiveStatus ? <PortalCompactTodayStatus job={todayBookingWithLiveStatus} booking={todayLeadflowBooking} savingNote={updateLeadflowJobCustomerNote.isPending} onUpdateNote={todayLeadflowBooking ? note => updateLeadflowJobCustomerNote.mutate({ id: todayLeadflowBooking.id, note }) : undefined} onViewBooking={openTodayBooking} /> : null} onGoToPage={goToPage} onMessageTeam={startMessageForNextBooking} onRequestScheduleChange={() => { if (nextCustomerBooking) setScheduleRequestBooking(nextCustomerBooking); }} onRequestExtras={() => { if (nextCustomerBooking) setExtrasRequestBooking(nextCustomerBooking); }} canMessageTeamToday={messageEligibleLeadflowJobs.length > 0} onBookHomeCleaning={() => setShowCleaningRebook(true)} onOpenService={openService} />{scheduleRequestBooking && <CustomerPortalScheduleRequest booking={scheduleRequestBooking} onClose={() => setScheduleRequestBooking(null)} />}{extrasRequestBooking && <CustomerPortalExtrasRequest booking={extrasRequestBooking} onClose={() => setExtrasRequestBooking(null)} />}{selectedService && <ServiceRequestForm service={selectedService} homeAddress={homeAddress} savedCard={portal.data.savedCard} customerName={portal.data.account.name} onClose={() => setSelectedService(null)} />}{showCleaningRebook && <div className="mib-portal-rebook-overlay" role="dialog" aria-modal="true" aria-label="Rebook home cleaning"><BookNow portalRebook={{ customerName: portal.data.account.name, phone: portal.data.account.phone, email: portal.data.account.email, address: homeAddress, savedCard: portal.data.savedCard }} onClose={closeCleaningRebook} /></div>}</>;
  }
