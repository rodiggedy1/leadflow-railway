...
import { Bell, CheckCheck, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, FileText, Filter, MessageSquare, Pause, Pencil, Play, RefreshCw, Search, Send, Sparkles } from "lucide-react";
...
export default function ConfirmationCallsExactLive() {
  ...
  const { data, isLoading, isFetching, refetch } = trpc.confirmationCalls.getJobsForDay.useQuery(
    { date },
  ...
  const dispatch = useCallback(async () => {
    ...
    try {
      for (const job of toCall) {
        ...
        try {
          await placeCall.mutateAsync({ cleanerJobId: job.id, jobDate: date, clientName: job.customerName ?? "Client", calledPhone: job.customerPhone });
        } catch (error) {
  ...
  return <main className="confirmation-review ops-review">
    ...
    <div className="ops-content ops-content--narrow">
      ...
      {tab === "dispatch" ? <section className="ops-confirmation-list">
        <header className="ops-list-head"><div><span>Dispatch queue</span><h2>Confirmation messages</h2><p>{jobs.length} appointments · <b>{results.length} sent</b> · {pending.length} remaining</p></div><button className="ops-quiet" onClick={selectAll}>{allSelected ? "Deselect all" : "Select all unsent"}</button></header>
        ...
        {isLoading ? <div className="ops-empty-state">Loading confirmation queue…</div> : jobs.length === 0 ? <div className="ops-empty-state">No appointments are scheduled for this date.</div> : <div className="ops-dispatch-stack">{jobs.map((job) => {
          const status = displayStatus(job.confirmationCall);
          const isPending = !job.confirmationCall || job.confirmationCall.status === "pending";
          return <label className={`ops-dispatch-card ${selected.includes(job.id) ? "is-selected" : ""}`} key={job.id}><input type="checkbox" disabled={!isPending} checked={selected.includes(job.id)} onChange={() => toggle(job.id)} /><span className="ops-time"><b>{formatTime(job.serviceDateTime)}</b><small>{job.teamName ?? "—"}</small></span><span className="ops-confirm-person"><b>{job.customerName ?? "Unknown"}</b><small>{job.jobAddress ?? "—"}</small><i>{job.serviceType ?? "—"}</i></span><em className={`ops-confirm-status ${status.className}`}>{status.label}</em></label>;
        })}</div>}
