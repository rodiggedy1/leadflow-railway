...
function JobDrawer({
  ...
  const uploadPhotos = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const [index, file] of files.entries()) {
      ...
      await uploadMutation.mutateAsync({ cleanerJobId: job.cleanerJobId, completedJobId: job.completedJobId, filename: file.name, mimeType: file.type || "image/jpeg", dataBase64, photoType: photos.length === 0 && index === 0 ? "before" : "after" });
    }
    setUploading(false);
    event.target.value = "";
  };
  ...
  return (
    <div className="cp-drawer-backdrop" onClick={onClose}>
      <aside className="cp-drawer" onClick={event => event.stopPropagation()} aria-label={`Details for ${job.customerName}`}>
        ...
        <section className="cp-detail-block"><div className="cp-block-heading"><div><h3>Before & after photos</h3><p>Capture the visit condition and finished result.</p></div><button className="cp-btn cp-btn--subtle cp-btn--small" onClick={() => photoInputRef.current?.click()} disabled={uploading}>{uploading ? <Loader2 className="cp-spin" size={15} /> : <Camera size={15} />}Add photos</button></div><input className="cp-hidden-input" ref={photoInputRef} type="file" accept="image/*" capture="environment" multiple onChange={uploadPhotos} />
          <div className="cp-photo-grid"><button className="cp-photo-tile" onClick={() => photoInputRef.current?.click()}><ImagePlus size={20} /><span>Before</span></button><button className="cp-photo-tile cp-photo-tile--after" onClick={() => photoInputRef.current?.click()}><ImagePlus size={20} /><span>After</span></button>{photos.map((url, index) => <img key={url} src={url} className="cp-photo-preview" alt={`Uploaded job photo ${index + 1}`} />)}</div>
        </section>
