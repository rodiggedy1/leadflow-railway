...
function JobDrawer({ job, onClose, onProgress, onContact }: { job: PortalJob; onClose: () => void; onProgress: (progress: { jobStatus: string; etaTimestamp: number | null; etaTimeStr: string | null }) => void; onContact: () => void }) {
  ...
  const setEtaMutation = trpc.cleanerPortalProgress.setEta.useMutation({ throwOnError: false, onSuccess: result => { onProgress(result); setEtaOpen(false); result.notificationAlreadySent ? toast.info("This ETA was already recorded; the client was not messaged again.") : result.customerNotified ? toast.success("ETA recorded and client notified.") : toast.warning(result.notificationError ? "ETA recorded, but the client message could not be sent." : "ETA recorded. No customer phone is on this booking."); }, onError: error => toast.error(error.message || "The ETA could not be recorded.") });
  const arrivedMutation = trpc.cleanerPortalProgress.markArrived.useMutation({ throwOnError: false, onSuccess: result => { onProgress(result); setArrivalConfirm(false); result.notificationAlreadySent ? toast.info("Arrival already recorded; the client was not messaged again.") : result.customerNotified ? toast.success("Arrival recorded and client notified.") : toast.warning(result.notificationError ? "Arrival recorded, but the client message could not be sent." : "Arrival recorded. No customer phone is on this booking."); }, onError: error => toast.error(error.message || "Arrival could not be recorded.") });
