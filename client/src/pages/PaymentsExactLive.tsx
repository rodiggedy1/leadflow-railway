...
export default function PaymentsExactLive() {
  ...
  const defaultSelected =
    visible.find(record => record.kind === "authorization" && record.state === "held") ??
    visible.find(record => record.kind === "card") ??
    visible[0] ??
    records.find(record => record.kind === "authorization" && record.state === "held") ??
    records.find(record => record.kind === "card") ??
    records[0] ??
    null;
  const selected = records.find(record => record.id === selectedId) ?? defaultSelected;
