...
export default function PaymentsExactLive() {
  ...
  return <main className="payments-review payments-live-exact" data-live-payments="true">
    ...
    <div className="payments-content">
      <section className="payments-page-head"><div><span className="payments-eyebrow">Finance & billing · Live workspace</span><h1><CreditCard size={27} />Payments</h1><p>One operational view for existing card-link, saved-card, authorization, capture, and release workflows.</p></div><button type="button" className="payments-new" onClick={() => { setShowLinkForm(true); setGeneratedLink(null); }}><Plus size={15} />Create payment link</button></section>
      <p className="payments-notice"><Sparkles size={14} />Live records use the existing guarded Stripe workflows. Card numbers are never displayed here.</p>
      ...
      <section className="payments-workbench" aria-label="Live payment workspace">
        ...
        {selected ? <article className={`payment-detail payment-detail--${selected.state}`}><header className="payment-detail-head"><div className="payment-detail-person"><CustomerPortrait customer={selected.customer} className="payment-portrait payment-portrait--detail" /><div><span className="payments-eyebrow">Selected payment context</span><h2>{selected.customer}</h2><p>{selected.phone}</p></div></div><div className="payment-detail-state"><em className={`payment-status payment-status--${selected.state}`}>{selected.statusLabel}</em><strong>{selected.kind === "authorization" ? `AUTH-${selected.authorizationId}` : selected.kind === "card" ? "CARD ON FILE" : "CARD LINK"}</strong></div></header><section className="payment-total-band"><div><span>{selected.kind === "authorization" ? "Amount" : "Record type"}</span><strong>{selected.kind === "authorization" ? formatCents(selected.amountCents) : selected.kind === "card" ? "Saved card" : "Secure link"}</strong></div><p>{selected.method}<i />{selected.invoice}</p></section><section className="payment-detail-grid"><div className="payment-detail-summary"><span>Payment context</span><p>{selected.detail}</p><small><LockKeyhole size={13} />Card data remains outside the application and is handled by Stripe.</small></div><div className="payment-detail-card"><span>Payment method</span><strong>{selected.card}</strong><p>{selected.method}</p></div></section><section className="payment-lifecycle"><header><span>Payment lifecycle</span><small>Existing record</small></header><div><i className="is-created" /><p><strong>Payment record created</strong><span>{selected.created}</span></p></div><div><i className={selected.state === "captured" ? "is-captured" : selected.state === "failed" ? "is-failed" : selected.state === "review" ? "is-review" : selected.state === "held" ? "is-held" : "is-released"} /><p><strong>{selected.statusLabel}</strong><span>{selected.note}</span></p></div></section>{selected.errorMessage && <section className="payment-live-warning"><CircleAlert size={15} /><span>{selected.errorMessage}</span></section>}{selected.kind === "authorization" && <section className="payment-existing-fields"><span>Existing authorization details</span><div><p><small>Job</small>{selected.jobLabel || "—"}</p><p><small>Authorized</small>{formatDate(selected.authorizedAt)}</p><p><small>Captured</small>{formatDate(selected.capturedAt)}</p><p><small>Released</small>{formatDate(selected.cancelledAt)}</p><p><small>Created by</small>{selected.createdBy || "—"}</p><p><small>Action by</small>{selected.actionBy || "—"}</p></div></section>}{selected.kind === "card" && <section className="payment-live-panel"><header><span>Existing preauthorization workflow</span><button type="button" className="payment-action" onClick={() => setPreauthOpen(open => !open)}><Zap size={14} />{preauthOpen ? "Cancel" : "Preauthorize"}</button></header>{preauthOpen && <form onSubmit={submitPreauth} className="payment-live-form"><label><DollarSign size={13} />Amount (USD) *<input type="number" min="0.50" step="0.01" value={preauthAmount} onChange={(event) => setPreauthAmount(event.target.value)} placeholder="150.00" required autoFocus /></label><label>Job label<input value={preauthJobLabel} onChange={(event) => setPreauthJobLabel(event.target.value)} placeholder="Deep clean — July 10" /></label><label>Notes<input value={preauthNotes} onChange={(event) => setPreauthNotes(event.target.value)} placeholder="Optional" /></label><button type="submit" className="payment-action is-primary" disabled={createPreauth.isPending}>{createPreauth.isPending ? <Loader2 className="animate-spin" /> : <DollarSign size={14} />}Place hold</button></form>}</section>}{selected.kind === "authorization" && selected.state === "held" && <section className="payment-live-panel"><header><span>Existing authorization actions</span><em>Action only after an explicit click</em></header><div className="payment-live-capture"><label>Capture amount (optional)<input type="number" min="0.50" step="0.01" value={captureAmount} onChange={(event) => setCaptureAmount(event.target.value)} placeholder={selected.amountCents ? (selected.amountCents / 100).toFixed(2) : "0.00"} /></label><button type="button" className="payment-action is-primary" onClick={captureSelected} disabled={capturePayment.isPending}>{capturePayment.isPending ? <Loader2 className="animate-spin" /> : <CheckCircle2 size={14} />}Capture</button><button type="button" className="payment-action is-danger" onClick={cancelSelected} disabled={cancelPreauth.isPending}>{cancelPreauth.isPending ? <Loader2 className="animate-spin" /> : <XCircle size={14} />}Cancel hold</button></div></section>}{selected.kind === "link" && selected.token && <section className="payment-live-panel"><header><span>Existing secure card link</span><CopyButton text={`https://quote.maidinblack.com/pay/${selected.token}`} label="Copy link" /></header><p>The original active-link copy behavior is preserved. Copying does not change the link or card record.</p></section>}<section className="payment-safety"><ShieldCheck size={15} /><div><strong>Existing card safeguards remain in force</strong><p>All Stripe queries, card-link creation, preauthorization, capture, cancellation, access checks, and confirmation behavior are unchanged.</p></div></section><footer className="payment-action-dock"><div><span>Recommended next step</span><strong>{selected.kind === "authorization" && selected.state === "held" ? "Confirm readiness before a payment decision" : selected.kind === "card" ? "Review the customer context before placing a hold" : selected.kind === "link" ? "Use the established secure-link workflow" : "Review the existing payment context before a customer follow-up"}</strong></div><div>{selected.kind === "link" && selected.token && <CopyButton text={`https://quote.maidinblack.com/pay/${selected.token}`} label="Copy link" />}{selected.kind === "authorization" && <button type="button" className="payment-action" onClick={() => void authorizations.refetch()}><RefreshCw size={14} />Refresh</button>}{selected.kind === "card" && <button type="button" className="payment-action" onClick={() => void cards.refetch()}><RefreshCw size={14} />Refresh</button>}</div></footer></article> : <article className="payment-detail payment-detail-empty"><CreditCard size={28} /><h2>No payment record selected</h2><p>Select an existing payment, saved card, or card link.</p><button type="button" className="payments-new" onClick={() => setShowLinkForm(true)}><Plus size={14} />Create payment link</button></article>}
      </section>
    </div>
