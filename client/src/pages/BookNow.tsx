...
type BookNowProps = {
  embedded?: boolean;
  onClose?: () => void;
  portalRebook?: { customerName: string; phone: string; email: string | null; address: string; savedCard: { brand: string | null; last4: string } | null };
};

export default function BookNow({ embedded = false, onClose, portalRebook }: BookNowProps) {
  ...
  const reuseSavedCardMutation = trpc.bookingPayments.reuseSavedCard.useMutation();
  ...
  const isPanel = embedded || Boolean(portalRebook);
  const nameParts = portalRebook?.customerName.trim().split(/\s+/).filter(Boolean) ?? [];
  ...
  const [phone, setPhone] = useState(portalRebook?.phone ?? "");
  const [email, setEmail] = useState(portalRebook?.email ?? "");
  const [address, setAddress] = useState(portalRebook?.address ?? "");
  ...
  const [savedCardChoice, setSavedCardChoice] = useState<"saved" | "new">(portalRebook?.savedCard ? "saved" : "new");
  const [savedCardReservationError, setSavedCardReservationError] = useState("");
  const [done, setDone] = useState(false);
  const [showOtherDates, setShowOtherDates] = useState(false);
  const captureLeadOnOpen = embedded || Boolean(portalRebook);
    ...
    const capture = (async () => {
      ...
      if (!current) {
        current = await beginFunnelMutation.mutateAsync({
          ...
          portalLeadCard: portalRebook ? true : undefined,
          customerName: fullName,
          customerPhone: phone,
        });
  ...
  useEffect(() => {
    if (!captureLeadOnOpen || funnelRecordRef.current || leadCapturePromiseRef.current) return;
  ...
  }, [captureLeadOnOpen, fullName, phone]);
  ...
  const completePortalSavedCardReservation = async () => {
    const current = funnelRecordRef.current;
    if (!current) return;
    setSavedCardReservationError("");
    try {
      await reuseSavedCardMutation.mutateAsync({
        publicFunnelNumber: current.publicFunnelNumber,
        mutationToken: current.mutationToken,
      });
      setCardOnFile(true);
      ...
      setSavedCardReservationError(error instanceof Error ? error.message : "We could not reserve your appointment with this card. Please try again or choose a new card.");
    }
  };
  ...
  const resetDemo = () => {
    ...
    setStep(1);
    setCardOnFile(false);
    setSavedCardChoice(portalRebook?.savedCard ? "saved" : "new");
    setSavedCardReservationError("");
  ...
  const pageClassName = isPanel ? `mib-booking-panel${portalRebook ? " mib-portal-rebook-panel" : ""}` : "book-now-page";
  ...
  if (done) {
    return <main className={`${pageClassName} book-now-confirmation-page`}><section className="book-now-confirmation-card"><span className="book-now-big-check"><Check /></span><small>BOOKING CONFIRMED</small><h1>You&apos;re booked, {firstName} 🎉</h1><p>Your cleaning is booked. We&apos;ll text and email your appointment details shortly.</p><div className="book-now-confirmation-details"><div><CalendarDays /><span><small>WHEN</small><strong>{date.full} · {time}</strong></span></div><div><MapPin /><span><small>WHERE</small><strong>{address || "Service address"}</strong></span></div></div><div className="book-now-expect-grid"><article><span>📩</span><strong>Helpful reminders</strong><p>We&apos;ll text you before your cleaning.</p></article><article><span>🚗</span><strong>Track your team</strong><p>Get a tracking link when they&apos;re on the way.</p></article><article><span>💳</span><strong>Pay after service</strong><p>Your card won&apos;t be charged until the cleaning is complete.</p></article></div><button type="button" onClick={onClose ?? resetDemo}>{portalRebook ? "Return to My Home" : embedded ? "Return to Madison" : "Start over"}</button></section></main>;
  }
  ...
  return <main ref={embedded ? embeddedPanelRef : undefined} className={pageClassName}>
    {isPanel ? (
      <>
        <header className="mib-booking-panel__header">
          <div className="mib-booking-panel__agent"><span className="mib-booking-panel__avatar">M<i /></span><div><strong>{portalRebook ? "Maids in Black" : "Madison"}</strong><span>{portalRebook ? "Rebook your home cleaning" : "Here to help"}</span></div></div>
        ...
        <section className="mib-booking-panel__intro">
          <button type="button" onClick={onClose} aria-label={portalRebook ? "Return to My Home" : "Return to Madison"} className="mib-booking-panel__back"><ArrowLeft className="h-4 w-4" />{portalRebook ? "Back to My Home" : "Back to chat"}</button>
          <span>{portalRebook ? "REBOOK HOME CLEANING" : "Book home cleaning"} · Step {step} of 4</span>
          <h1>{step === 1 ? "Build your cleaning." : step === 2 ? "Choose your time." : step === 3 ? "Almost there." : "Secure your appointment."}</h1>
          <p>{step === 1 ? "Choose what you need. Your price updates instantly." : step === 2 ? "Choose the day and arrival window that works for you." : step === 3 ? "We’ll use this information for confirmation and arrival updates." : portalRebook?.savedCard ? "Choose your saved card or add a different one. You won’t be charged today." : "Add a card to reserve your cleaning. You won’t be charged today."}</p>
    ...
    <div className="book-now-layout"><section className="book-now-workspace">
      ...
      {step === 3 && <div className="book-now-step-panel"><div className="book-now-step-heading"><small>YOUR DETAILS</small><h1>Almost there.</h1><p>We&apos;ll use this information for confirmation and arrival updates.</p></div><div className="book-now-form-grid"><label><span>First name</span><input value={firstName} onChange={(event) => setFirstName(event.target.value)} onBlur={() => void saveLeadContactIfReady()} autoComplete="given-name" /></label><label><span>Last name</span><input value={lastName} onChange={(event) => setLastName(event.target.value)} onBlur={() => void saveLeadContactIfReady()} autoComplete="family-name" /></label><label><span>Mobile phone</span><input value={phone} onChange={(event) => setPhone(event.target.value)} onBlur={() => void saveLeadContactIfReady()} inputMode="tel" autoComplete="tel" /></label><label><span>Email address</span><input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" /></label>{portalRebook && portalRebook.address && !useDifferentAddress ? <div className="book-now-saved-address full"><span>Service address</span><strong><MapPin />{portalRebook.address}</strong><button type="button" onClick={() => setUseDifferentAddress(true)}>Use a different address</button></div> : <label className="full"><span>Service address</span><div className="book-now-input-icon"><MapPin /><input value={address} onChange={(event) => setAddress(event.target.value)} autoComplete="street-address" /></div>{portalRebook?.address && <button type="button" className="book-now-inline-text-action" onClick={() => { setAddress(portalRebook.address); setUseDifferentAddress(false); }}>Use my home-cleaning address</button>}</label>}<label className="full"><span>Access details or special requests <em>Optional</em></span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Gate code, pets, priorities, parking instructions…" /></label></div>{formError && <div className="book-now-form-error" role="alert">{formError}</div>}{funnelRecord && <div className="book-now-save-status"><Check />{funnelRecord.stage === "lead" ? "Information saved" : "Reservation saved"}</div>}<div className="book-now-privacy-note"><LockKeyhole /><div><strong>Your information stays private</strong><p>We only use it for your booking, reminders, and arrival updates.</p></div></div></div>}
      {step === 4 && <div className="book-now-step-panel"><div className="book-now-step-heading"><small>SECURE YOUR APPOINTMENT</small><h1>{portalRebook?.savedCard ? "Use your card on file." : "Add a card to reserve your spot."}</h1><p>{portalRebook?.savedCard ? "Review your saved card or choose a different one. You won&apos;t be charged today." : "You won&apos;t be charged until after your cleaning is complete."}</p></div>{funnelRecord ? <BookingPaymentCheckout publicFunnelNumber={funnelRecord.publicFunnelNumber} mutationToken={funnelRecord.mutationToken} customerName={fullName} amountCents={Math.round(priceBreakdown.total * 100)} savedCard={portalRebook?.savedCard} footerReservesSavedCard={Boolean(portalRebook?.savedCard)} selectedPaymentChoice={savedCardChoice} onPaymentChoiceChange={(choice) => { setSavedCardChoice(choice); setSavedCardReservationError(""); }} externalSavedCardError={savedCardReservationError} onComplete={(result) => { setCardOnFile(true); if (portalRebook) { setDone(true); return; } if (embedded && window.parent !== window) { if (!result.portalAccessCode) return; window.parent.postMessage({ type: "mib-booking-widget-portal", code: result.portalAccessCode }, "https://maidsinblack.com"); return; } if (result.directPortalSessionReady) { window.location.assign("/my-home"); } }} /> : <div className="book-now-form-error" role="alert">Your reservation needs to be saved before secure card entry.</div>} {cardOnFile && <div className="book-now-save-status"><Check />Card saved securely. Your cleaning is booked. You won&apos;t be charged today.</div>}</div>}
      <footer className="book-now-step-actions">{step > 1 ? <button type="button" className="book-now-back-button" disabled={funnelMutationPending || reuseSavedCardMutation.isPending} onClick={() => setStep((current) => current - 1)}><ArrowLeft />Back</button> : <span />}<button type="button" className="book-now-next-button" disabled={(step === 3 && funnelMutationPending) || (step === 4 && !cardOnFile && !(portalRebook?.savedCard && savedCardChoice === "saved")) || reuseSavedCardMutation.isPending} onClick={() => step === 3 ? void continueToSecureBooking() : step < 4 ? setStep((current) => current + 1) : portalRebook?.savedCard && savedCardChoice === "saved" && !cardOnFile ? void completePortalSavedCardReservation() : setDone(true)}>{step === 4 && reuseSavedCardMutation.isPending ? "Completing reservation…" : step === 4 ? "Complete reservation" : step === 3 && funnelMutationPending ? "Saving reservation…" : step === 3 ? "Continue to secure booking" : "Continue"}<ArrowRight /></button></footer>
