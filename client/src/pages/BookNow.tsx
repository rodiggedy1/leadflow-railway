...
export default function BookNow({ embedded = false, onClose }: BookNowProps) {
  ...
  const ensurePortalFromFunnelMutation = trpc.customerPortal.ensureFromFunnel.useMutation();
  ...
  const captureLeadIfReady = async (showValidation = false): Promise<BookingFunnelPublicResult | null> => {
    ...
    const capture = (async () => {
      ...
      if (!initialSnapshotSavedRef.current) {
        ...
        void ensurePortalFromFunnelMutation.mutateAsync({ publicFunnelNumber: current.publicFunnelNumber, mutationToken: current.mutationToken }).catch(() => undefined);
      }
      setFormError("");
      return current;
    })().catch((error: unknown) => {
  ...
  return <main ref={embedded ? embeddedPanelRef : undefined} className={pageClassName}>
    ...
    <div className="book-now-layout"><section className="book-now-workspace">
      ...
      {step === 4 && <div className="book-now-step-panel"><div className="book-now-step-heading"><small>SECURE YOUR APPOINTMENT</small><h1>Add a card to reserve your spot.</h1><p>You won&apos;t be charged until after your cleaning is complete.</p></div>{funnelRecord ? <BookingPaymentCheckout publicFunnelNumber={funnelRecord.publicFunnelNumber} mutationToken={funnelRecord.mutationToken} customerName={fullName} amountCents={Math.round(priceBreakdown.total * 100)} onComplete={(result) => { setCardOnFile(true); if (!result.portalHandoffCode) return; const portalUrl = `${window.location.origin}/my-home?access=${encodeURIComponent(result.portalHandoffCode)}`; if (!embedded || window.parent === window) { window.location.assign(portalUrl); return; } const parentOrigin = document.referrer ? new URL(document.referrer).origin : window.location.origin; window.parent.postMessage({ type: "mib-customer-portal-open", url: portalUrl }, parentOrigin); }} /> : <div className="book-now-form-error" role="alert">Your reservation needs to be saved before secure card entry.</div>} {cardOnFile && <div className="book-now-save-status"><Check />Card saved securely. Your cleaning is booked. You won&apos;t be charged today.</div>}</div>}
