...
export default function BookNow({ embedded = false, onClose, portalRebook }: BookNowProps) {
  ...
  return <main ref={embedded ? embeddedPanelRef : undefined} className={pageClassName}>
    ...
    <div className="book-now-layout"><section className="book-now-workspace">
      ...
      {step === 4 && <div className="book-now-step-panel"><div className="book-now-step-heading"><small>SECURE YOUR APPOINTMENT</small><h1>{portalRebook?.savedCard ? "Use your card on file." : "Add a card to reserve your spot."}</h1><p>You won&apos;t be charged until after your cleaning is complete.</p></div>{funnelRecord ? <BookingPaymentCheckout publicFunnelNumber={funnelRecord.publicFunnelNumber} mutationToken={funnelRecord.mutationToken} customerName={fullName} amountCents={Math.round(priceBreakdown.total * 100)} savedCard={portalRebook?.savedCard} onComplete={(result) => { setCardOnFile(true); if (portalRebook) return; if (embedded && window.parent !== window) { if (!result.portalAccessCode) return; window.parent.postMessage({ type: "mib-booking-widget-portal", code: result.portalAccessCode }, "https://maidsinblack.com"); return; } if (result.directPortalSessionReady) { window.location.assign("/my-home"); } }} /> : <div className="book-now-form-error" role="alert">Your reservation needs to be saved before secure card entry.</div>} {cardOnFile && <div className="book-now-save-status"><Check />Card saved securely. Your cleaning is booked. You won&apos;t be charged today.</div>}</div>}
