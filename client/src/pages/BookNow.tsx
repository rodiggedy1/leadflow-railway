...
export default function BookNow() {
  ...
  const customerName = [firstName.trim(), lastName.trim()].filter(Boolean).join(" ") || "there";
  ...
  return <main className="book-now-page">
    ...
    <div className="book-now-layout"><section className="book-now-workspace">
      ...
      <footer className="book-now-step-actions">{step > 1 ? <button type="button" className="book-now-back-button" onClick={() => setStep((current) => current - 1)}><ArrowLeft />Back</button> : <span />}<button type="button" className="book-now-next-button" disabled={step === 4 && !demoCardSaved} onClick={() => step < 4 ? setStep((current) => current + 1) : setDone(true)}>{step === 4 ? "Preview confirmation" : step === 3 ? "Continue to secure booking" : "Continue"}<ArrowRight /></button></footer>
