      ...
      <section className="ccc-workspace">
        <div className="ccc-command-grid">
          ...
          <section className="ccc-command-panel ccc-center-panel">
            ...
            <div className="ccc-reference-chat-tabs"><button type="button" className={view === "chat" ? "active" : ""} onClick={() => setView("chat")}>Messages</button><button type="button" className={view === "issues" ? "active" : ""} onClick={() => setView("issues")}>Issues <b>{openIssues.length}</b></button><button type="button" className={view === "calls" ? "active" : ""} onClick={() => { setThreadId(null); setThreadDraft(""); setView("calls"); }}>Threads <b>{activeThreads.length}</b></button></div>
            {view === "issues" ? <IssuesView issues={openIssues} onBack={() => setView("chat")} /> : view === "calls" ? <ThreadsView threads={activeThreads} onOpen={(id) => { setThreadDraft(""); setThreadId(id); }} onBack={() => setView("chat")} /> : <>
...
function ThreadsView({ threads, onOpen, onBack }: { threads: Array<{ parentId: number; parentFrom: string; parentBody: string; replyCount: number; lastReplyBody: string | null; lastReplyTs: number; hasUnread: boolean }>; onOpen: (id: number) => void; onBack: () => void }) {
  return <div className="ccc-alt-view ccc-live-threads-view"><header><div><span>COMMAND THREADS</span><h2>{threads.length ? `${threads.length} active discussions` : "No active threads"}</h2></div><button type="button" onClick={onBack}>Back to chat</button></header>{threads.length ? <section className="ccc-live-threads-list" aria-label="Active command threads">{threads.map((thread) => <button type="button" className="ccc-live-threads-row" key={thread.parentId} onClick={() => onOpen(thread.parentId)}><span><MessageSquare /></span><div><strong>{thread.parentFrom}</strong><small>{thread.lastReplyBody || thread.parentBody}</small></div><b className={thread.hasUnread ? "queued" : ""}>{thread.replyCount} {thread.replyCount === 1 ? "reply" : "replies"}</b><ChevronRight /></button>)}</section> : <div className="ccc-live-empty"><MessageSquare />Start a thread from any command message.</div>}</div>;
}
