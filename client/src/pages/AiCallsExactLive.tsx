...
function customerToItem(customer: CustomerRow): PersonItem {
  return {
    id: `c-${customer.leadflowJobId}`,
    leadflowJobId: customer.leadflowJobId,
    name: customer.name,
...
function TemplateEditor({ template, onClose, onSaved }: { template: Partial<TemplateRow> & { scenario: string; audience: Audience }; onClose: () => void; onSaved: () => void }) {
  ...
  const upsert = trpc.leadflowJobs.callMatrix.upsertTemplate.useMutation({ onSuccess: () => { onSaved(); onClose(); } });
...
function TemplatesPanel() {
  const { data: templates, isLoading, refetch } = trpc.leadflowJobs.callMatrix.getTemplates.useQuery(undefined, { staleTime: 30_000 });
  const deleteTemplate = trpc.leadflowJobs.callMatrix.deleteTemplate.useMutation({ onSuccess: () => refetch() });
...
export default function AiCallsExactLive() {
  ...
  const { data: peopleData, isLoading: peopleLoading, error: peopleError } = trpc.leadflowJobs.callMatrix.getPeople.useQuery({ date }, { staleTime: 60_000 });
  const { data: templates } = trpc.leadflowJobs.callMatrix.getTemplates.useQuery(undefined, { staleTime: 30_000 });
  const { data: matrixCallHistory, isLoading: matrixHistoryLoading, error: matrixHistoryError } = trpc.leadflowJobs.callMatrix.getCallHistory.useQuery({ limit: 50 }, { staleTime: 30_000 });
  ...
  const matchScenarioMutation = trpc.leadflowJobs.callMatrix.matchScenario.useMutation();
  const startCallMutation = trpc.leadflowJobs.callMatrix.startCall.useMutation({
    onSuccess: (result) => {
  ...
  function startPolling(vapiCallId: string) {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const result = await utils.leadflowJobs.callMatrix.pollCall.fetch({ vapiCallId });
  ...
  function confirmAndFire() {
    ...
    startCallMutation.mutate({
      leadflowJobId: selectedPerson.leadflowJobId,
      jobDate: date,
