...
function PhotoUploadButton({
  ...
  job: {
    ...
    photos: Array<{ id: number; photoUrl: string; thumbnailUrl: string | null; filename: string | null; photoType?: string }>;
  };
  onSuccess: () => void;
}) {
  ...
  if (hasPhoto) {
    ...
    const generalPhotos = photos.filter(p => !p.photoType || p.photoType === "general");
    const currentPhoto = lightboxIndex !== null ? photos[lightboxIndex] : null;

    const renderStrip = (group: typeof photos, label: string, labelColor: string) => (
      <div className="mb-2">
        <div className="flex items-center gap-1.5 mb-1.5">
          <Images className="w-3 h-3 text-slate-400" />
          <span className={`text-[10px] font-semibold uppercase tracking-wide ${labelColor}`}>{label} ({group.length})</span>
        </div>
        <div className="flex items-center gap-1.5 flex-wrap">
          {group.slice(0, 5).map((p, i) => {
          ...
          {group.length > 5 && (
            <button
              onClick={() => setLightboxIndex(photos.indexOf(group[5]))}
            ...
            >
              <span className="text-xs font-bold text-slate-600">+{group.length - 5}</span>
      ...
      <>
        {photos.length > 0 ? (
          <div>
            ...
            {beforePhotos.length > 0 && renderStrip(beforePhotos, "Before", "text-sky-600")}
            {afterPhotos.length > 0 && renderStrip(afterPhotos, "After", "text-emerald-600")}
            {generalPhotos.length > 0 && (beforePhotos.length > 0 || afterPhotos.length > 0
              ? renderStrip(generalPhotos, "General", "text-slate-500")
              : (
                <div className="flex items-center gap-1.5 flex-wrap">
                  {generalPhotos.slice(0, 5).map((p, i) => {
                    const globalIdx = photos.indexOf(p);
                    return (
                      <button key={p.id} onClick={() => setLightboxIndex(globalIdx)}
                        className="relative group w-12 h-12 rounded-md overflow-hidden border border-slate-200 hover:border-emerald-400 transition-all flex-shrink-0">
                        <img src={p.thumbnailUrl ?? p.photoUrl} alt={p.filename ?? `Photo ${i + 1}`}
                          className="w-full h-full object-cover" loading="lazy" decoding="async" width={48} height={48} />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                          <ZoomIn className="w-3 h-3 text-white opacity-0 group-hover:opacity-100 transition-opacity" />
                        </div>
                      </button>
                  ...
                  {generalPhotos.length > 5 && (
                    <button onClick={() => setLightboxIndex(photos.indexOf(generalPhotos[5]))}
                      className="w-12 h-12 rounded-md border border-slate-200 bg-slate-100 hover:bg-slate-200 transition-all flex-shrink-0 flex flex-col items-center justify-center gap-0.5">
                      <span className="text-xs font-bold text-slate-600">+{generalPhotos.length - 5}</span>
                      <span className="text-[9px] text-slate-400 leading-none">more</span>
                    </button>
                  )}
                </div>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-5">
          ...
          <div className="flex-1 min-w-0 space-y-2">
            ...
            {/* Customer Sign-off — signature + response */}
            {(job.cleanerAssignment?.signatureUrl || job.cleanerAssignment?.customerResponse || job.cleanerAssignment?.customerNotHome) && (
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 space-y-2">
                <div className="flex items-center gap-1.5">
                  {job.cleanerAssignment.customerNotHome ? (
                    <>
                ...
                {job.cleanerAssignment.customerResponse && (
                  <p className="text-xs text-slate-700 italic">"{job.cleanerAssignment.customerResponse}"</p>
                )}
                {job.cleanerAssignment.signatureUrl && (
                  <a href={job.cleanerAssignment.signatureUrl} target="_blank" rel="noopener noreferrer"
                    className="block mt-1">
                    <img
                      src={job.cleanerAssignment.signatureUrl}
                      alt="Customer signature"
                      className="h-10 max-w-[160px] object-contain rounded border border-slate-200 bg-white p-1"
                    />
                  </a>
                )}
              </div>
