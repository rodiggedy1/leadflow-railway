/**
 * LeadflowScheduleCallLogPanel — AI Call Command Center
 *
...
function CallCard({ entry, onNotesUpdated }: { entry: CallLogEntry; onNotesUpdated: () => void }) {
  ...
  const updateCallLog = trpc.leadflowScheduleCalls.updateCallLog.useMutation({
    onSuccess: () => {
...
interface LeadflowScheduleCallLogPanelProps {
  open: boolean;
  onClose: () => void;
  jobDate: string;
}

export default function LeadflowScheduleCallLogPanel({ open, onClose, jobDate }: LeadflowScheduleCallLogPanelProps) {
  const { data: entries = [], refetch, isLoading } = trpc.leadflowScheduleCalls.getCallLog.useQuery(
    { jobDate, limit: 50 },
