/**
 * TeamEtaModal.tsx
 * Pixel-perfect implementation matching team-eta-react-tailwind.zip design.
 * The design reference file at /home/ubuntu/eta-design/team-eta-react/TeamEtaModal.tsx
 * was used as the structural and visual source of truth.
 */
...
// ── State styles (directly from design reference) ─────────────────────────────
...
// ── Sub-components (matching design reference structure exactly) ───────────────

function HouseIcon({ roof = "#8FB7E7", muted = false, completed = false }: {
  roof?: string; muted?: boolean; completed?: boolean;
}) {
  return (
...
function ConfidenceRing({ value, color }: { value: number; color: string }) {
  ...
  return (
    <div className="relative h-[76px] w-[76px]">
      ...
      <div className="absolute inset-0 grid place-items-center text-base font-extrabold">
        {value ? `${value}%` : "—"}
      </div>
    </div>
  );
...
function Timeline({ team }: { team: TeamEtaSummaryItem }) {
  ...
  const currentIdx = Math.max(0, jobs.findIndex(j =>
    j.jobStatus !== "completed" && j.jobStatus !== "cancelled"
  ));
  const progress = jobs.length <= 1 ? 0 : (currentIdx / (jobs.length - 1)) * 100;
  ...
  return (
    <div className="relative min-w-0 flex-1 px-5 pb-2 pt-3">
      ...
      <div
        ...
        style={{
          width: `calc(${Math.max(progress, 5)}% - 8%)`,
          background: `linear-gradient(90deg,#22C55E,${s.accent})`,
        }}
      />
      <div
...
function SummaryCard({ icon, count, label, helper, color, soft }: {
  ...
  return (
    <div className="flex min-w-[185px] flex-1 items-center gap-3 rounded-[20px] border border-slate-200/80 bg-white px-4 py-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
      <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl" style={{ background: soft, color }}>
        {icon}
      </div>
      <div>
      ...
      <div className="flex h-10 flex-1 items-center gap-[3px]">
        {[5, 11, 17, 10, 20, 26, 18, 31, 22, 14, 27, 34, 20, 12, 25, 18, 30, 13, 22, 10].map((h, i) => (
          <span
            ...
            style={{ height: h, background: "linear-gradient(to top, #a5b4fc, #6366f1)" }}
          />
        ))}
      </div>
...
function ExpandedCard({ team }: { team: TeamEtaSummaryItem }) {
  ...
  const scheduledTime = formatTime(team.currentJobServiceDateTime);
  const etaTimeStr = team.etaCall?.etaTimeStr ?? null;
  const confidence = team.etaConfidence ?? 0;

  // Footer text
  const footerText = team.etaStatus === "unclear" || team.etaStatus === "no_answer"
    ? "Final route completion is still unclear"
    : team.completedAt
    ? `Completed at ${formatTime(team.completedAt)}`
    : etaTimeStr
    ? `ETA ${etaTimeStr}`
    : scheduledTime
  ...
  return (
    ...
    >
      {/* Top row: team identity | timeline | confidence ring */}
      <div className="grid grid-cols-[250px_minmax(0,1fr)_110px] gap-2 border-b border-slate-100 px-5 py-5">
        ...
        <div className="relative border-r border-slate-100 pr-5">
          {/* Accent stripe */}
          <div
            className="absolute -left-5 -top-5 h-[calc(100%+40px)] w-1 rounded-r-full"
          ...
          <div className="mt-5 space-y-2 text-sm text-slate-500">
            <div className="flex items-center gap-2">
              <Users className="h-4 w-4" /> {team.jobs.length} jobs today
            </div>
            <div className="flex items-center gap-2">
              <CarFront className="h-4 w-4" /> {team.cleanerName}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center border-l border-slate-100 pl-3">
          ...
          <div className="mt-1 text-center text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400">
            ETA confidence
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 bg-slate-50/70 p-5">
        ...
        <section className="rounded-[22px] border p-5" style={{ background: s.soft, borderColor: s.border }}>
          <div className="text-[11px] font-extrabold uppercase tracking-[0.16em]" style={{ color: s.text }}>
            ETA &amp; status
          </div>
          ...
          >
            ...
            {team.delayMinutes > 0 ? `Running ${team.delayMinutes} min late` : s.label}
          </div>
        ...
        <section className="rounded-[22px] border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-5">
          ...
          {/* Speech bubble with cleaner avatar */}
          <div className="mt-4 flex items-start gap-3">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-indigo-200 text-sm font-extrabold text-indigo-700">
              {getInitials(team.cleanerName)}
            </div>
            <div>
              <div className="text-xs font-bold text-slate-500 mb-1">
                {team.cleanerName}
                {team.etaCall?.createdAt ? (
                  <span className="ml-2 font-normal text-slate-400">{formatTime(new Date(team.etaCall.createdAt))}</span>
                ) : null}
              </div>
              <div className="rounded-[20px] rounded-tl-md bg-indigo-100/80 p-4 text-[15px] font-semibold italic leading-6">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <span
            ...
            style={{ background: s.soft, color: s.text }}
          >
            {team.delayMinutes > 0 ? `${team.delayMinutes} min late` : s.label}
          </span>
        </div>
      </div>
      ...
      <div className="flex min-w-0 items-center">
        {jobs.map((job, idx) => {
          const isCurrent = idx === currentIdx;
          const isDone = idx < currentIdx || job.jobStatus === "completed";
          return (
            <React.Fragment key={job.id}>
              ...
              {idx < jobs.length - 1 && (
                <div
                  ...
                  style={{ background: idx < currentIdx ? s.accent : "#D9DEE7" }}
                />
              )}
            </React.Fragment>
          );
...
export function TeamEtaModal({ open, onClose }: TeamEtaModalProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="flex h-[calc(100vh-40px)] w-full max-w-[1540px] flex-col overflow-hidden rounded-[30px] border border-white/80 shadow-[0_34px_100px_rgba(15,23,42,0.22)] backdrop-blur-xl"
      ...
      >
        ...
        <main className="min-h-0 flex-1 overflow-y-auto px-8 pb-8">
          ...
          <section
            className="sticky top-0 z-20 -mx-8 px-8 pb-4 pt-5"
            style={{ background: "rgba(247,249,252,0.95)", backdropFilter: "blur(12px)" }}
          >
            {/* Summary stat cards */}
            <div className="flex gap-3 overflow-x-auto pb-1">
              ...
              <SummaryCard icon={<AlertTriangle className="h-5 w-5" />} count={counts.critical} label="Significantly Late" helper="30+ min late" color="#EF4444" soft="#FFF2F2" />
