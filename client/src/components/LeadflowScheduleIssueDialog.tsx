/**
 * LeadflowScheduleIssueDialog — AI Call Command Center
 *
...
interface LeadflowScheduleIssueDialogProps {
  open: boolean;
  onClose: () => void;
...
export default function LeadflowScheduleIssueDialog({
  open, onClose, leadflowJobId, jobDate, onCallFired,
}: LeadflowScheduleIssueDialogProps) {
  ...
  // ── Server mutations ─────────────────────────────────────────────────────
  const raiseIssue = trpc.leadflowScheduleCalls.raiseIssue.useMutation();
  const fireCall = trpc.leadflowScheduleCalls.fireCall.useMutation({
    onSuccess: (data) => {
      toast.success("Call placed successfully");
      utils.leadflowScheduleCalls.getCallLog.invalidate({ jobDate });
      utils.leadflowScheduleCalls.getDayIssues.invalidate({ jobDate });
  ...
  // ── Pre-fill only directly owned contact data ─────────────────────────────────
  useEffect(() => {
    if (!jobInfo) return;
    if (callTarget === "team") {
      // Scheduler teams do not store an owned phone number, so the dispatcher
      // enters the intended team contact in the editable field below.
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {callTarget === "team" && jobInfo?.teamName && (
                <p className="text-xs text-gray-400 mt-1">Team: {jobInfo.teamName} — enter the call number for this issue.</p>
              )}
