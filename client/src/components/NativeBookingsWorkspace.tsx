...
const labelStatus = (value: string) => value === "lead" ? "Lead / In progress" : value === "needs_attention" ? "Needs attention" : value === "pending_payment" ? "Pending payment" : value.charAt(0).toUpperCase() + value.slice(1);
...
export default function NativeBookingsWorkspace() {
  ...
  const funnelListInput = useMemo(() => ({ stage: "lead" as const, query: query.trim() || undefined, limit: 200 }), [query]);
  ...
  const rows = useMemo<WorkspaceRow[]>(() => {
    ...
    if (view === "bookings") return bookingRows;
    return funnelLeads.map((lead) => ({
            key: `funnel:${lead.id}`,
  ...
  return <main className={`bookings-ops-shell ${active ? "has-detail" : ""}`}>
    ...
    <section className="bookings-ops-main">
      ...
      <div className="bookings-list" aria-label={view === "bookings" ? "Native LeadFlow bookings list" : "Native LeadFlow leads list"}><div className="bookings-list-head"><span>TIME & CUSTOMER</span><span>SERVICE</span><span>TEAM</span><span>PAYMENT</span><span>TOTAL</span><span /></div>{(view === "bookings" ? listQuery.isLoading : funnelListQuery.isLoading) ? <div className="bookings-empty-day"><Loader2 className="animate-spin" /><h3>Loading {view}</h3></div> : (view === "bookings" ? listQuery.error : funnelListQuery.error) ? <div className="bookings-empty-day"><X /><h3>Could not load {view}</h3><p>{view === "bookings" ? listQuery.error?.message : funnelListQuery.error?.message}</p></div> : rows.length ? rows.map((row) => { const home = row.bedrooms === null || row.bathrooms === null ? "Details in progress" : row.bedrooms === 0 ? `Studio · ${row.bathrooms} baths` : `${row.bedrooms} bed · ${row.bathrooms} baths`; const hasCard = row.paymentStatus === "card_on_file"; return <button type="button" className={activeKey === row.key ? "bookings-row selected" : "bookings-row"} key={row.key} onClick={() => setActiveKey(row.key)}><span className="bookings-customer-cell"><b>{row.requestedLocalTime ? displayTime(row.requestedLocalTime) : "Lead"}</b><i className={row.status === "lead" || row.status === "needs_attention" ? "bookings-status-dot attention" : "bookings-status-dot"} /><span><strong>{row.customerName}</strong><small><MapPin /> {row.address ?? "Details in progress"}</small></span></span><span className="bookings-service-cell"><strong>{row.serviceName ?? "Booking lead"}</strong><small>{home}{row.recurrence ? ` · ${labelRecurrence(row.recurrence)}` : ""}</small>{row.source === "funnel" ? <em>Lead / In progress</em> : row.extras.length > 0 && <em>+{row.extras.length} extra{row.extras.length > 1 ? "s" : ""}</em>}</span><span className="bookings-team-cell"><i className="bookings-team-avatar gray">?</i><span><strong>Unassigned</strong><small>Needs review</small></span></span><span className={hasCard ? "bookings-payment-ok" : "bookings-payment-missing"}><CreditCard />{hasCard ? "Card on file" : "Not started"}</span><strong className="bookings-row-price">{row.firstCleaningTotalCents === null ? "—" : `$${(row.firstCleaningTotalCents / 100).toFixed(0)}`}</strong><MoreHorizontal /></button>; }) : <div className="bookings-empty-day"><CalendarDays /><h3>No {view} found</h3><p>{view === "bookings" ? "Try another date or clear your filters." : "New phone-captured leads will appear here."}</p></div>}</div>
    </section>
