...
export default function NativeBookingsWorkspace({ realtimeEnabled }: { realtimeEnabled: boolean }) {
  ...
  const rows = useMemo<WorkspaceRow[]>(() => {
    const bookingRows = bookings.map((booking) => ({
      ...
      assignmentStatus: booking.assignmentStatus, paymentStatus: booking.paymentStatus, paymentBrand: null, paymentLast4: null,
    ...
    const funnelRows = funnelLeads.filter((lead) => !lead.bookingId).map((lead) => ({
      ...
      assignmentStatus: "unassigned", paymentStatus: lead.paymentLast4 ? "card_on_file" : "not_started", paymentBrand: null,
    ...
    const portalRequestRows = portalRequests.map((request) => ({
      ...
      extras: [], specialRequestNotes: request.customerRequest ? [request.customerRequest] : [], assignmentStatus: "unassigned",
    ...
    const importedRows = (leadflowJobsQuery.data ?? []).map((job) => ({
      ...
      assignmentStatus: job.teamName ? "assigned" : "unassigned", paymentStatus: "not_started", paymentBrand: null,
      paymentLast4: null, stripePaymentMethodId: null, paymentChargedAt: null, firstCleaningTotalCents: job.jobTotalCents,
    }));
    const scheduledRows = [...funnelRows.filter((row) => row.status !== "lead"), ...bookingRows, ...importedRows]
  ...
  return <main className={`bookings-ops-shell ${active ? "has-detail" : ""}`}>
    <section className="bookings-ops-main">
      <header className="bookings-ops-header"><div><p>OPERATIONS · BOOKINGS</p><h1>Bookings</h1><span>{view === "bookings" ? "Native requests and isolated Launch27 imports appear here for review." : "Phone-captured booking leads appear here while customers finish the flow."}</span>{importSummary && <p>{importSummary}</p>}{importLeadflowJobs.error && <p role="alert">Import failed: {importLeadflowJobs.error.message}</p>}</div><button type="button" className="bookings-new-booking" disabled title="Manual booking creation is not connected in this release"><Plus />New booking</button><button type="button" className="bookings-new-booking" disabled={importLeadflowJobs.isPending} onClick={() => importLeadflowJobs.mutate(undefined, { onSuccess: (result) => { setImportSummary(`30-day import: ${result.totals.active} active jobs; ${result.totals.created} added; ${result.totals.updated} refreshed; ${result.totals.errors} day errors.`); void leadflowJobsQuery.refetch(); } })}><CalendarDays />{importLeadflowJobs.isPending ? "Importing…" : "Import next 30 days"}</button></header>
