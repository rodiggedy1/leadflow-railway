...
export default function NativeBookingsWorkspace({ realtimeEnabled }: { realtimeEnabled: boolean }) {
  ...
  const assignableTeamsQuery = trpc.bookings.listAssignableTeams.useQuery(undefined, { enabled: activeKey?.startsWith("booking:") === true, staleTime: 60_000 });
  ...
  const rows = useMemo<WorkspaceRow[]>(() => {
    const bookingRows = bookings.filter((booking) => booking.status !== "cancelled").map((booking) => ({
    ...
    const portalRequestRows = portalRequests.filter((request) => request.status !== "cancelled").map((request) => ({
    ...
    const importedRows = (leadflowJobsQuery.data ?? []).filter((job) => job.bookingStatus.toLowerCase() !== "cancelled").map((job) => ({
    ...
    const scheduledRows = [...funnelRows.filter((row) => row.status !== "lead" && row.status !== "cancelled"), ...bookingRows, ...importedRows]
  ...
  const cards = rows.filter((row) => row.paymentStatus === "card_on_file" || row.paymentStatus === "captured").length;
