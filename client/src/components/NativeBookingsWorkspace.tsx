...
type WorkspaceRow = {
  key: string;
  source: "booking" | "funnel" | "portal_request";
  id: number;
  publicNumber: string;
  customerName: string;
...
export default function NativeBookingsWorkspace({ realtimeEnabled }: { realtimeEnabled: boolean }) {
  ...
  const portalRequestListInput = useMemo(() => ({ query: query.trim() || undefined, limit: 200 }), [query]);
  ...
  const portalRequestListQuery = trpc.customerPortal.adminRequests.useQuery(portalRequestListInput, { staleTime: 10_000 });
  ...
  const refreshBookingAndFunnelQueries = () => {
    ...
    void portalRequestListQuery.refetch();
  ...
  const rows = useMemo<WorkspaceRow[]>(() => {
    ...
    const portalRequestRows = status === "All" ? (portalRequestListQuery.data ?? []).map((request) => ({
      key: `portal-request:${request.id}`,
      source: "portal_request" as const,
      id: request.id,
      publicNumber: request.publicRequestNumber,
      customerName: request.customerName,
      customerPhone: request.customerPhone,
      customerEmail: request.customerEmail,
      status: request.status,
      requestedLocalDate: request.requestedLocalDate,
      requestedLocalTime: request.requestedLocalTime,
      ...
      recurrence: null,
      extras: [],
      specialRequestNotes: [request.customerRequest],
      assignmentStatus: "unassigned",
      paymentStatus: "not_started",
      firstCleaningTotalCents: request.estimatedTotalCents,
    })) : [];
  ...
  }, [bookings, date, funnelLeads, portalRequestListQuery.data, status, view]);
  ...
  return <main className={`bookings-ops-shell ${active ? "has-detail" : ""}`}>
    <section className="bookings-ops-main">
      ...
      <div className="bookings-list" aria-label={view === "bookings" ? "Native LeadFlow bookings list" : "Native LeadFlow leads list"}><div className="bookings-list-head"><span>TIME & CUSTOMER</span><span>SERVICE</span><span>TEAM</span><span>PAYMENT</span><span>TOTAL</span><span /></div>{(listQuery.isLoading || funnelListQuery.isLoading || portalRequestListQuery.isLoading) ? <div className="bookings-empty-day"><Loader2 className="animate-spin" /><h3>Loading {view}</h3></div> : (listQuery.error || funnelListQuery.error || portalRequestListQuery.error) ? <div className="bookings-empty-day"><X /><h3>Could not load {view}</h3><p>{listQuery.error?.message ?? funnelListQuery.error?.message ?? portalRequestListQuery.error?.message}</p></div> : rows.length ? rows.map((row) => { const home = row.source === "portal_request" ? "Service request" : row.bedrooms === null || row.bathrooms === null ? "Details in progress" : row.bedrooms === 0 ? `Studio · ${row.bathrooms} baths` : `${row.bedrooms} bed · ${row.bathrooms} baths`; const hasCard = row.paymentStatus === "card_on_file"; return <button type="button" className={activeKey === row.key ? "bookings-row selected" : "bookings-row"} key={row.key} onClick={() => setActiveKey(row.key)}><span className="bookings-customer-cell"><b>{row.requestedLocalTime ? displayTime(row.requestedLocalTime) : row.source === "portal_request" ? "Request" : "Lead"}</b><i className={row.status === "lead" || row.status === "needs_attention" || row.source === "portal_request" ? "bookings-status-dot attention" : "bookings-status-dot"} /><span><strong>{row.customerName}</strong><small><MapPin /> {row.address ?? "Details in progress"}</small></span></span><span className="bookings-service-cell"><strong>{row.serviceName ?? "Booking lead"}</strong><small>{home}{row.recurrence ? ` · ${labelRecurrence(row.recurrence)}` : ""}</small>{row.source === "funnel" || row.source === "portal_request" ? <em>{labelStatus(row.status)}</em> : row.extras.length > 0 && <em>+{row.extras.length} extra{row.extras.length > 1 ? "s" : ""}</em>}</span><span className="bookings-team-cell"><i className="bookings-team-avatar gray">?</i><span><strong>Unassigned</strong><small>Needs review</small></span></span><span className={hasCard ? "bookings-payment-ok" : "bookings-payment-missing"}><CreditCard />{hasCard ? "Card on file" : "Not started"}</span><strong className="bookings-row-price">{row.firstCleaningTotalCents === null ? "—" : `$${(row.firstCleaningTotalCents / 100).toFixed(0)}`}</strong><MoreHorizontal /></button>; }) : <div className="bookings-empty-day"><CalendarDays /><h3>No {view} found</h3><p>{view === "bookings" ? "Try another date or clear your filters." : "New phone-captured leads will appear here."}</p></div>}</div>
    </section>
