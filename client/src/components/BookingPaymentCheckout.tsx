...
type BookingPaymentCheckoutProps = {
  ...
  footerReservesSavedCard?: boolean;
...
export function BookingPaymentCheckout({ publicFunnelNumber, mutationToken, customerName, amountCents, savedCard, footerReservesSavedCard = false, selectedPaymentChoice, onPaymentChoiceChange, externalSavedCardError = "", onComplete }: BookingPaymentCheckoutProps) {
  ...
  const [internalPaymentChoice, setInternalPaymentChoice] = useState<"saved" | "new">(savedCard ? "saved" : "new");
  const paymentChoice = selectedPaymentChoice ?? internalPaymentChoice;
  const selectPaymentChoice = (choice: "saved" | "new") => {
    setCheckoutError("");
    setInternalPaymentChoice(choice);
  ...
  return (
    <div className="rounded-[20px] border border-[#e4e5e7] bg-white p-4 shadow-[0_14px_36px_rgba(22,20,33,0.08)] sm:p-5">
      ...
      <div className="mt-4 flex gap-3 rounded-xl border border-[#cfe9df] bg-[#f3fbf7] p-3 text-[10px] leading-5 text-[#41695a]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#168d61]" /><p>{savedCard ? "Choose the saved card below or add a new card. You won’t be charged until after service." : "Add a card to reserve your cleaning. You won’t be charged until after service. Your payment information is securely handled by Stripe and is never stored on our servers."}</p></div>
      {savedCard && !clientSecret && <div className="mt-4 grid gap-2" role="radiogroup" aria-label="Payment method"><button type="button" role="radio" aria-checked={paymentChoice === "saved"} onClick={() => selectPaymentChoice("saved")} className={`rounded-xl border p-3 text-left ${paymentChoice === "saved" ? "border-[#ff684c] bg-[#fff7f4] ring-2 ring-[#ff684c]/10" : "border-[#e4e5e7] bg-white"}`}><strong className="block text-[12px] text-[#3a3c41]">Use {savedCard.brand ? `${savedCard.brand} ` : "card "}ending in {savedCard.last4}</strong><span className="mt-1 block text-[10px] text-[#6f7279]">Selected by default · no charge today</span></button><button type="button" role="radio" aria-checked={paymentChoice === "new"} onClick={() => selectPaymentChoice("new")} className={`rounded-xl border p-3 text-left ${paymentChoice === "new" ? "border-[#ff684c] bg-[#fff7f4] ring-2 ring-[#ff684c]/10" : "border-[#e4e5e7] bg-white"}`}><strong className="block text-[12px] text-[#3a3c41]">Add a new card</strong><span className="mt-1 block text-[10px] text-[#6f7279]">Use a different payment method for this rebook</span></button></div>}
      {!clientSecret ? <div className="mt-4 space-y-3">{paymentChoice === "saved" && savedCard ? (footerReservesSavedCard ? <div className="flex items-center gap-2 rounded-xl border border-[#cfe9df] bg-[#f3fbf7] px-3 py-3 text-[11px] font-semibold text-[#41695a]" role="status"><ShieldCheck className="h-4 w-4 shrink-0 text-[#168d61]" />Your saved card is selected. Complete your reservation below.</div> : <button type="button" onClick={() => void reserveWithSavedCard()} disabled={reuseSavedCard.isPending} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#ff684c] px-4 py-3 text-[12px] font-extrabold text-white transition hover:bg-[#e9573e] disabled:cursor-not-allowed disabled:opacity-50"><LockKeyhole className="h-4 w-4" />{reuseSavedCard.isPending ? "Reserving with saved card…" : `Reserve with card ending in ${savedCard.last4} →`}</button>) : <><label className="flex cursor-pointer items-start gap-2.5 rounded-xl bg-[#f8f7f5] p-3 text-[10px] leading-5 text-[#5f6168]"><input type="checkbox" checked={consentAccepted} onChange={(event) => setConsentAccepted(event.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 accent-[#ff684c]" /><span>{BOOKING_PAYMENT_CONSENT_TEXT}</span></label><button type="button" onClick={() => void beginCardCollection()} disabled={!consentAccepted || startSetup.isPending} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#ff684c] px-4 py-3 text-[12px] font-extrabold text-white transition hover:bg-[#e9573e] disabled:cursor-not-allowed disabled:opacity-50"><LockKeyhole className="h-4 w-4" />{startSetup.isPending ? "Preparing secure card entry…" : "Continue to secure card entry →"}</button></>}</div> : <Elements stripe={stripePromise} options={{ clientSecret, appearance: { theme: "stripe" } }}><div className="mt-4"><ExistingCardSetupForm clientSecret={clientSecret} customerName={customerName} onConfirm={completeCardCollection} /></div></Elements>}
      {(checkoutError || externalSavedCardError) && <p role="alert" className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[11px] font-semibold text-red-700">{checkoutError || externalSavedCardError}</p>}
    </div>
  );
}
