...
export function BookingPaymentCheckout({ publicFunnelNumber, mutationToken, customerName, amountCents, savedCard, autoUseSavedCard = false, onComplete }: BookingPaymentCheckoutProps) {
  ...
  const savedCardAutoReservationStarted = useRef(false);
  ...
  useEffect(() => {
    if (!autoUseSavedCard || !savedCard || paymentChoice !== "saved" || savedCardAutoReservationStarted.current) return;
    savedCardAutoReservationStarted.current = true;
    void reserveWithSavedCard();
  }, [autoUseSavedCard, savedCard, paymentChoice]);

  return (
    <div className="rounded-[20px] border border-[#e4e5e7] bg-white p-4 shadow-[0_14px_36px_rgba(22,20,33,0.08)] sm:p-5">
      <div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#fff1ed] text-[#e9573e]"><CreditCard className="h-4.5 w-4.5" /></span><div><h3 className="text-[15px] font-extrabold text-[#3a3c41]">{autoUseSavedCard ? "Saved payment method" : "Secure card on file"}</h3><p className="mt-0.5 text-[10px] text-[#6f7279]">{autoUseSavedCard ? "Securely on file · no charge today" : `$${(amountCents / 100).toFixed(0)} after service · Stripe secure`}</p></div></div>
      <div className="mt-4 flex gap-3 rounded-xl border border-[#cfe9df] bg-[#f3fbf7] p-3 text-[10px] leading-5 text-[#41695a]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#168d61]" /><p>{autoUseSavedCard ? "Your saved card is securely on file and will reserve this appointment. You won’t be charged until after service." : "Add a card to reserve your cleaning. You won’t be charged until after service. Your payment information is securely handled by Stripe and is never stored on our servers."}</p></div>
      {autoUseSavedCard && savedCard ? <div className="mt-4 space-y-3"><div className="flex items-center gap-2 rounded-xl border border-[#cfe9df] bg-[#f3fbf7] px-3 py-3 text-[11px] font-semibold text-[#41695a]" role="status"><ShieldCheck className="h-4 w-4 shrink-0 text-[#168d61]" />{reuseSavedCard.isPending ? "Reserving your appointment with your saved card…" : "Your saved card is securely on file. Finalizing your appointment…"}</div>{checkoutError && <button type="button" onClick={() => void reserveWithSavedCard()} disabled={reuseSavedCard.isPending} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#ff684c] bg-white px-4 py-3 text-[12px] font-extrabold text-[#d94b35] transition hover:bg-[#fff7f4] disabled:cursor-wait disabled:opacity-60">{reuseSavedCard.isPending ? "Retrying reservation…" : "Try again"}</button>}</div> : <>{savedCard && !clientSecret && <div className="mt-4 grid gap-2" role="radiogroup" aria-label="Payment method"><button type="button" role="radio" aria-checked={paymentChoice === "saved"} onClick={() => setPaymentChoice("saved")} className={`rounded-xl border p-3 text-left ${paymentChoice === "saved" ? "border-[#ff684c] bg-[#fff7f4] ring-2 ring-[#ff684c]/10" : "border-[#e4e5e7] bg-white"}`}><strong className="block text-[12px] text-[#3a3c41]">Use {savedCard.brand ? `${savedCard.brand} ` : "card "}ending in {savedCard.last4}</strong><span className="mt-1 block text-[10px] text-[#6f7279]">Selected by default · no charge today</span></button><button type="button" role="radio" aria-checked={paymentChoice === "new"} onClick={() => setPaymentChoice("new")} className={`rounded-xl border p-3 text-left ${paymentChoice === "new" ? "border-[#ff684c] bg-[#fff7f4] ring-2 ring-[#ff684c]/10" : "border-[#e4e5e7] bg-white"}`}><strong className="block text-[12px] text-[#3a3c41]">Add a new card</strong><span className="mt-1 block text-[10px] text-[#6f7279]">Use a different payment method for this rebook</span></button></div>}
