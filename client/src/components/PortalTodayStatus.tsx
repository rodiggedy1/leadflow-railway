...
export function PortalTodayStatus({ job, booking, onViewBooking, onUpdateNote, savingNote = false }: { job: CustomerPortalTodayJob; booking: PortalBookingDetail | null; onViewBooking: () => void; onUpdateNote?: (note: string) => void; savingNote?: boolean }) {
  ...
  return <section className={`mib-direct-live-status${view.isRunningLate ? " is-running-late" : ""}`} aria-label="Today’s cleaning status" aria-live="polite">
    ...
    <div className="mib-direct-live-details"><div><span><NotebookPen aria-hidden="true" /> TODAY&apos;S SERVICE</span><strong>{serviceName}</strong><p>{formatScheduledTime(serviceTime)}{booking?.bedrooms ? ` · ${booking.bedrooms} bedroom${booking.bedrooms === 1 ? "" : "s"}` : ""}{booking?.bathrooms ? ` · ${booking.bathrooms} bath${booking.bathrooms === 1 ? "" : "s"}` : ""}</p></div>{address && <div><span><MapPin aria-hidden="true" /> SERVICE ADDRESS</span><strong>{address}</strong></div>}{booking && onUpdateNote ? <form onSubmit={event => { event.preventDefault(); if (noteChanged) onUpdateNote(note); }}><label htmlFor={`portal-today-note-${booking.id}`}>Notes for today</label><textarea id={`portal-today-note-${booking.id}`} value={note} onChange={event => setNote(event.target.value)} maxLength={2_000} placeholder="Add a note for your cleaning team" /><button type="submit" disabled={!noteChanged || savingNote}>{savingNote ? "Saving note…" : "Save note"}</button></form> : noteValue ? <div className="mib-direct-live-note"><span>NOTES FOR TODAY</span><p>{noteValue}</p></div> : null}</div>
  </section>;
}
