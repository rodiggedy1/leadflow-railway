...
describe("Customer Profile exact review shell", () => {
  ...
  it("mounts the customer profile destination without global runtime chrome", () => {
    ...
    expect(app).toContain("function AdminCustomerProfileExactReviewRoute() {");
    expect(app).toContain('<ReviewWorkspaceFrame navActivePath="/review/customer-profile"><CustomerProfileExactLive /></ReviewWorkspaceFrame>');
