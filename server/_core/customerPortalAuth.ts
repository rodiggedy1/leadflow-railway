...
export type CustomerPortalSession = { accountId: number; customerName: string; customerPhone: string };
...
export async function getCustomerPortalSessionFromRequest(req: Request): Promise<CustomerPortalSession | null> {
  return verifyCustomerPortalSession(parseCookieHeader(req.headers.cookie ?? "")[CUSTOMER_PORTAL_COOKIE_NAME]);
}
