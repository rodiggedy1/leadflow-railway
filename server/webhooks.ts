...
async function handleCallAnswered(event: any): Promise<void> {
  ...
  // Broadcast so the Command Chat header and message list update in real time
  const { broadcastOpsUpdate } = await import("./sseBroadcast");
...
async function handleCallCompleted(event: any): Promise<void> {
  ...
  const db = await getDb();
  if (!db) return;
  // Find the agent who was on this call before clearing (so we can name them in the card)
  const [agent] = await db
  ...
  // Post a call_ended card to the command channel
  if (agent) {
...
async function handleCallSummaryCompleted(event: any): Promise<void> {
  // call.summary.completed fires after call.completed — use as a secondary clear signal
  const obj = event?.data?.object;
  const callId: string | undefined = obj?.callId ?? obj?.id;
  if (!callId) return;
  const db = await getDb();
  if (!db) return;
  // Clear on-call status if still set (handles cases where call.completed was missed)
  const [agent] = await db
    .select({ id: agents.id, name: agents.name })
    .from(agents)
  ...
  if (agent) {
    await db
      ...
      .set({ onCallSince: null, onCallCallId: null } as any)
      .where(eq(agents.id, agent.id));
    console.log(`[CallStatus] call.summary.completed cleared on-call status for ${agent.name} (callId=${callId})`);
    const { broadcastOpsUpdate } = await import("./sseBroadcast");
    broadcastOpsUpdate("agent_status");
  }
}
