...
async function ownedActiveLeadflowJob(cleanerId: number, portalJobKey: string) {
  ...
  const jobs = await db.select({ id: leadflowJobs.id, customerName: leadflowJobs.customerName, customerPhone: leadflowJobs.customerPhone, customerEmail: leadflowJobs.customerEmail }).from(leadflowJobs).where(and(
