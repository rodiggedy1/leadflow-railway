...
export const leadflowJobsRouter = router({
  ...
  /**
   * Read-only Operations Dashboard projection. The page needs live daily
   * scheduling, field-progress, lead, activity, and review signals, so this
   * procedure returns only those existing LeadFlow-owned records. It makes no
   * booking, team, lead, message, notification, or assignment mutations.
   */
  dashboardOverview: agentProcedure.input(z.object({
    ...
    for (const session of activeSessions) {
      try {
        ...
        // Invalid historical message JSON is excluded rather than guessed.
      }
    }
