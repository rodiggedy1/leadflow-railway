...
describe("AI Calls exact transcript workspace", () => {
  it("uses the approved transcript shell and retains the existing live call behavior", () => {
    ...
    expect(shell).toContain("trpc.leadflowJobs.callMatrix.getCallHistory.useQuery");
    ...
    expect(shell).toContain("trpc.leadflowJobs.callMatrix.getPeople.useQuery");
    expect(shell).toContain("trpc.leadflowJobs.callMatrix.getTemplates.useQuery");
    expect(shell).toContain("trpc.leadflowJobs.callMatrix.matchScenario.useMutation");
    expect(shell).toContain("trpc.leadflowJobs.callMatrix.startCall.useMutation");
    expect(shell).toContain("utils.leadflowJobs.callMatrix.pollCall.fetch");
    ...
    expect(app).toContain("function AdminAiCallsExactReviewRoute()");
    expect(app).toContain('<ReviewWorkspaceFrame navActivePath="/review/ai-calls-transcript"><AiCallsExactLive /></ReviewWorkspaceFrame>');
