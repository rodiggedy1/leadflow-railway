...
async function handleSendPaymentLink(
  ...
  if (resolvedClientPhone) {
    ...
    // Address is optional — the payment link works without it
  } else {
...
export const aiConciergeRouter = router({
  ...
  getReadinessSummary: agentProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
      try {
        // Resolve target date — default to tomorrow ET using canonical helper
        const { offsetServiceDate } = await import("./conciergeTime");
        const targetDate = input.date ?? offsetServiceDate(getTodayET(), 1);
        const summary = await computeReadinessSummary(db, targetDate);
        // Return the UI contract — omit the jobs array (UI only needs dimensions + scores)
        const { jobs: _jobs, ...uiResult } = summary;
        return uiResult;
      } catch (err) {
