...
export const qualityRouter = router({
  ...
  /**
   * quality.getDayOverview — returns a rich operations snapshot for a given date.
   * Powers the Operations Command Center tab.
   */
  getDayOverview: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const inProgress = jobs.filter(j => j.jobStatus === "in_progress").length;
      const onTheWay = jobs.filter(j => j.jobStatus === "on_the_way").length;
      const runningLate = jobs.filter(j => j.jobStatus === "running_late").length;
      const issueCount = jobs.filter(j => j.jobStatus === "issue_at_property").length;
      const notStarted = jobs.filter(j => !j.jobStatus || j.jobStatus === null).length;
      const photosSubmitted = jobs.filter(j => j.photoSubmitted === 1).length;
      const completedNoPhoto = jobs.filter(j => j.jobStatus === "completed" && j.photoSubmitted !== 1).length;
      const ratings = jobs.map(j => j.customerRating).filter((r): r is number => r !== null && r !== undefined);
      const avgRating = ratings.length > 0 ? Math.round((ratings.reduce((a, b) => a + b, 0) / ratings.length) * 10) / 10 : null;
      const totalRevenue = jobs.reduce((sum, j) => sum + (j.jobRevenue ? parseFloat(j.jobRevenue) : 0), 0);
      const totalPay = jobs.reduce((sum, j) => {
        const base = j.finalPay ? parseFloat(j.finalPay) : 0;
        const adj = j.manualAdjustment ? parseFloat(j.manualAdjustment) : 0;
        return sum + base + adj;
      }, 0);

      // ── Alerts ───────────────────────────────────────────────────────────────
      type Alert = {
        type: "running_late" | "overdue_eta" | "issue" | "no_photo" | "not_started";
      ...
      for (const j of jobs) {
        const cleanerName = j.cleanerName ?? "Unassigned";
        const serviceTime = j.serviceDateTime
          ? new Date(j.serviceDateTime).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })
          : null;

        if (j.jobStatus === "issue_at_property") {
          alerts.push({
            type: "issue",
            severity: "critical",
            jobId: j.id,
            ...
            serviceTime,
            message: j.issueNote ? `Issue: ${j.issueNote}` : "Issue reported at property",
            etaTimestamp: null,
            issueNote: j.issueNote ?? null,
          });
        }

        if ((j.jobStatus === "on_the_way" || j.jobStatus === "running_late") && j.etaTimestamp) {
          const isOverdue = j.etaTimestamp < now;
          const isDueSoon = !isOverdue && j.etaTimestamp - now < 10 * 60 * 1000;
          const etaStr = new Date(j.etaTimestamp).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true });
          ...
          if (isOverdue) {
            ...
            alerts.push({
              ...
              address: j.jobAddress ?? null,
              serviceTime,
              message: `Due soon — arriving ~${etaStr}`,
              etaTimestamp: j.etaTimestamp,
              issueNote: null,
            });
          }

          if (j.jobStatus === "running_late") {
            alerts.push({
              type: "running_late",
              ...
              address: j.jobAddress ?? null,
              serviceTime,
              message: `Running late — ETA ~${etaStr}`,
              etaTimestamp: j.etaTimestamp,
              issueNote: null,
            });
          }
        } else if (j.jobStatus === "running_late" && !j.etaTimestamp) {
          alerts.push({
            type: "running_late",
            severity: "warning",
            jobId: j.id,
            ...
            address: j.jobAddress ?? null,
            serviceTime,
            message: "Running late — ETA unknown",
            etaTimestamp: null,
            issueNote: null,
          });
        }

        if (j.jobStatus === "completed" && j.photoSubmitted !== 1) {
          alerts.push({
            type: "no_photo",
            severity: "info",
            jobId: j.id,
            ...
            address: j.jobAddress ?? null,
            serviceTime,
            message: "Completed but no photo submitted",
            etaTimestamp: null,
            issueNote: null,
          });
        }
      ...
      for (const j of jobs) {
        const key = j.cleanerName ?? "Unassigned";
        if (!cleanerMap.has(key)) {
          cleanerMap.set(key, { cleanerProfileId: j.cleanerProfileId, cleanerName: key, jobs: [] });
        }
        cleanerMap.get(key)!.jobs.push(j);
      }

      const teamStatus = Array.from(cleanerMap.values()).map(({ cleanerProfileId, cleanerName, jobs: cjobs }) => {
        const jobsDone = cjobs.filter(j => j.jobStatus === "completed").length;
        const totalJobs = cjobs.length;
        const hasIssue = cjobs.some(j => j.jobStatus === "issue_at_property");
        const isLate = cjobs.some(j => j.jobStatus === "running_late");
        const photosSubmitted = cjobs.filter(j => j.photoSubmitted === 1).length;
        const checklistProgress = cjobs.reduce((acc, j) => {
          const items = j.checklistItems ? (() => { try { return JSON.parse(j.checklistItems) as Array<{checked: boolean}>; } catch { return []; } })() : [];
          return { done: acc.done + items.filter(i => i.checked).length, total: acc.total + items.length };
        }, { done: 0, total: 0 });
        const latestStatus = cjobs
          .filter(j => j.jobStatus)
          .sort((a, b) => (b.serviceDateTime ?? "").localeCompare(a.serviceDateTime ?? ""))[0]?.jobStatus ?? null;
        const totalPay = cjobs.reduce((sum, j) => {
          const base = j.finalPay ? parseFloat(j.finalPay) : 0;
          const adj = j.manualAdjustment ? parseFloat(j.manualAdjustment) : 0;
          return sum + base + adj;
        }, 0);
        const ratings = cjobs.map(j => j.customerRating).filter((r): r is number => r !== null && r !== undefined);
        const avgRating = ratings.length > 0 ? Math.round((ratings.reduce((a, b) => a + b, 0) / ratings.length) * 10) / 10 : null;
        ...
        if (a.cleanerName === "Unassigned") return 1;
        if (b.cleanerName === "Unassigned") return -1;
      ...
      return {
        ...
        metrics: {
          ...
          totalPay: Math.round(totalPay * 100) / 100,
          netMargin: Math.round((totalRevenue - totalPay) * 100) / 100,
        },
        alerts,
        teamStatus,
      };
  ...
  /**
   * quality.generateDaySummary — uses LLM to produce a plain-English ops briefing.
   */
  generateDaySummary: protectedProcedure
    .input(z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }))
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
      ...
      const total = jobs.length;
      const completed = jobs.filter(j => j.jobStatus === "completed").length;
      const issues = jobs.filter(j => j.jobStatus === "issue_at_property");
      const late = jobs.filter(j => j.jobStatus === "running_late");
      const noPhoto = jobs.filter(j => j.jobStatus === "completed" && j.photoSubmitted !== 1);
      const ratings = jobs.map(j => j.customerRating).filter((r): r is number => r !== null);
      const avgRating = ratings.length > 0 ? (ratings.reduce((a, b) => a + b, 0) / ratings.length).toFixed(1) : "N/A";
      const totalRevenue = jobs.reduce((sum, j) => sum + (j.jobRevenue ? parseFloat(j.jobRevenue) : 0), 0);

      const prompt = `You are an operations manager for a professional cleaning company. Generate a concise daily briefing (3-5 sentences, plain English, no bullet points) for ${input.date} based on these stats:
- Total jobs: ${total}
- Completed: ${completed}/${total}
- Running late: ${late.length}${late.length > 0 ? " (" + late.map(j => j.cleanerName ?? "Unknown").join(", ") + ")" : ""}
- Issues at property: ${issues.length}${issues.length > 0 ? " (" + issues.map(j => (j.customerName ?? j.jobAddress ?? "Unknown") + ": " + (j.issueNote ?? "no details")).join("; ") + ")" : ""}
- Missing photos: ${noPhoto.length}
- Avg customer rating: ${avgRating}
- Total revenue: $${totalRevenue.toFixed(2)}

Be direct and actionable. Flag anything that needs immediate attention. If everything looks good, say so briefly.`;

      const response = await invokeLLM({
        messages: [
          { role: "system", content: "You are a concise operations manager writing a daily field service briefing." },
