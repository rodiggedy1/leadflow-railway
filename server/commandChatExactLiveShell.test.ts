...
describe("Command Chat exact live shell", () => {
  it("keeps the approved review composition while binding only existing Command Chat data and actions", () => {
    ...
    expect(page).toContain('import CsInbox2EmailWorkspace from "@/components/CsInbox2EmailWorkspace";');
    expect(page).toContain('function ExactEmailWorkspaceOverlay({ threadId, onClose }');
    expect(page).toContain('<CsInbox2EmailWorkspace key={threadId} initialThreadId={threadId} onCloseDetail={onClose} detailOnly />');
    ...
    expect(styles).toContain('.ccc-live-email-workspace-modal .em2-copy-detail-only{width:100%;height:100%;min-height:0;position:static}');
    expect(styles).toContain('.ccc-live-email-workspace-modal .em2-copy-detail-only>.em2-main{width:100%;height:100%;min-height:0}');
