...
describe("Command Chat exact live shell", () => {
  it("keeps the approved review composition while binding only existing Command Chat data and actions", () => {
    ...
    expect(allThreadsPanel).toContain('threads: unreadThreads.map((thread) => ({ parentId: thread.parentId, lastReplyMessageId: thread.lastReplyMsgId }))');
    expect(allThreadsPanel).toContain('Mark all read');
    expect(allThreadsPanel).toContain('bg-[#1b1b1e]');
    expect(opsChatRouter).toContain('markAllActiveThreadsRead: opsChatProcedure');
    expect(opsChatRouter).toContain('threads: z.array(z.object({');
    expect(opsChatRouter).toContain('eq(opsChatMessages.channel, "command")');
    expect(opsChatRouter).toContain('inArray(opsChatMessages.threadParentId, requestedParentIds)');
    expect(opsChatRouter).toContain('groupBy(opsChatMessages.threadParentId)');
    expect(opsChatRouter).toContain('await tx.execute(sql`');
    expect(opsChatRouter).toContain('if (inserts.length > 0) await tx.insert(opsChatReads).values(inserts);');
