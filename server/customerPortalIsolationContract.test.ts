...
describe("customer portal isolation", () => {
  ...
  it("shows the approved six-service portal preview with the remaining catalog behind View all services", async () => {
    ...
    expect(source).toContain('className="mib-direct-page mib-direct-services-page"');
    expect(source).toContain('className="mib-direct-services-featured"');
    expect(source).toContain('className="mib-direct-service-catalog"');
    expect(source).toContain("A clean home,<br />on your schedule.");
    expect(source).toContain("{CUSTOMER_PORTAL_SERVICES.length} services available");
    ...
    expect(source).toContain('"tv-mounting": "Mount my TV"');
    expect(source).toContain('"furniture-assembly": "Get it assembled"');
    expect(source).toContain('"picture-hanging": "Hang my pictures"');
    expect(source).toContain('"minor-home-repairs": "Fix it for me"');
    expect(source).toContain('handyman: "Book a handyman"');
    expect(source).toContain('plumbing: "Get plumbing help"');
    expect(source).toContain('"electrical-lighting": "Light it up"');
    expect(source).toContain('"interior-painting": "Paint my space"');
    ...
    expect(source).toContain('onClick={() => setShowCleaningRebook(true)}>Book cleaning <ArrowRight /></button>');
    expect(source).toContain('onClick={() => setSelectedService(service)}');
  ...
  it("uses the approved read-only same-day status panel while restoring Home statistics when no active job exists", async () => {
    ...
    expect(portal).toContain('sameDayJob ? <PortalTodayStatus job={sameDayJob} booking={sameDayLeadflowBooking}');
