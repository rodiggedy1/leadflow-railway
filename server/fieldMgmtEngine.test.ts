...
// ── placeNoCheckinEscalationCall — isTest flag ────────────────────────────────
// When isTest=true the business hours guard must be bypassed and the TTS script
// must be prefixed with a TEST notice.

describe("placeNoCheckinEscalationCall — isTest flag", () => {
  it("isTest parameter is accepted in the function signature", () => {
    const fs = require("fs");
    const path = require("path");
    const src = fs.readFileSync(
      path.resolve(__dirname, "fieldMgmtEngine.ts"),
    ...
    expect(src).toContain("isTest?: boolean;");
  });

  it("business hours guard is bypassed when isTest is true", () => {
    const fs = require("fs");
    const path = require("path");
    const src = fs.readFileSync(
      path.resolve(__dirname, "fieldMgmtEngine.ts"),
    ...
    // Guard must check !params.isTest before checking hours
    expect(src).toContain("if (!params.isTest && !isWithinEscalationHours())");
  });

  it("TTS script is prefixed with TEST notice when isTest is true", () => {
    const fs = require("fs");
    const path = require("path");
    const src = fs.readFileSync(
      path.resolve(__dirname, "fieldMgmtEngine.ts"),
    ...
    expect(src).toContain("This is a TEST call from Maids in Black. No action is required.");
    expect(src).toContain("const testPrefix = isTest ?");
  });

  it("testEscalationCall procedure is defined in fieldMgmtRouter.ts", () => {
    const fs = require("fs");
    const path = require("path");
    const src = fs.readFileSync(
    ...
    expect(src).toContain("testEscalationCall: adminProcedure");
    expect(src).toContain("overridePhone: z.string()");
    expect(src).toContain("isTest: true");
    expect(src).toContain("step: \"test_escalation_call\"");
  });

  it("testEscalationCall normalizes the override phone to E.164", () => {
    const fs = require("fs");
    const path = require("path");
    const src = fs.readFileSync(
      path.resolve(__dirname, "fieldMgmtRouter.ts"),
    ...
    // Must normalize to +1XXXXXXXXXX if no + prefix
    expect(src).toContain("normalizedOverride");
    expect(src).toContain("`+1${raw}`");
  });
});
