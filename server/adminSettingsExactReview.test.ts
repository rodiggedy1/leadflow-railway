...
describe("admin Settings exact review baseline", () => {
  it("renders the exact Settings live component inside the identical review frame", () => {
    expect(app).toContain("function AdminSettingsReviewRoute()");
    expect(app).toContain('<AdminPageGuard pageId="settings"><ReviewWorkspaceFrame navActivePath="/review/settings"><SettingsExactLive /></ReviewWorkspaceFrame></AdminPageGuard>');
  ...
  it("keeps the live access guard but excludes inherited runtime chrome from this exact baseline", () => {
    expect(app).toContain('const isSettingsWorkspace = location === "/admin/settings" || location === "/admin/widget-config";');
    expect(app).toContain("isHiringWorkspace || isSettingsWorkspace;");
    expect(app).toContain("if (isDayBoardExactLiveRoute(location)) return null;");
