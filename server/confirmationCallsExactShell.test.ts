...
describe("Confirmation Calls exact review shell", () => {
  ...
  it("mounts the exact shell inside the approved review workspace frame", () => {
    ...
    expect(appSource).toContain("function AdminConfirmationCallsExactReviewRoute() {");
    expect(appSource).toContain('<AdminPageGuard pageId="confirmation-calls"><ReviewWorkspaceFrame navActivePath="/review/confirmation-calls"><ConfirmationCallsExactLive /></ReviewWorkspaceFrame></AdminPageGuard>');
  ...
  it("does not retain the inherited live AdminHeader or runtime chrome on the exact route", () => {
    ...
    expect(appSource).toContain("if (isDayBoardExactLiveRoute(location)) return null;");
  });

  it("retains the existing live queue, send, override, recording, and transcript contracts", () => {
    expect(shellSource).toContain("trpc.leadflowConfirmationCalls.getJobsForDay.useQuery");
    expect(shellSource).toContain("trpc.leadflowConfirmationCalls.placeCall.useMutation");
    expect(shellSource).toContain("trpc.leadflowConfirmationCalls.overrideOutcome.useMutation");
