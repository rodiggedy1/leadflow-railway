  ...
  leads: router({
    ...
    markHandled: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db
          .update(conversationSessions)
          .set({ respondedAt: Date.now(), lastReadAt: sql`lastCustomerReplyAt` } as any)
          .where(eq(conversationSessions.id, input.sessionId));
        return { success: true };
      }),
...
Return JSON with exactly these fields:
{
  ...
  "action": "call" | "text" | "quote" | "follow_up"
    ...
    /**
     * leads.debugUnreadRows — diagnostic only.
     * Returns raw session fields for the rows that contribute to the unread chip count.
     * Used to verify whether unreadCount is stale or computed from the wrong session.
     * Remove after investigation is complete.
     */
    debugUnreadRows: publicProcedure
      .query(async () => {
        const db = await getDb();
        if (!db) return [];
        // Fetch all conversation sessions with non-null messageHistory
        const rows = await db
          .select({
            id: conversationSessions.id,
            leadPhone: conversationSessions.leadPhone,
        ...
        for (const row of rows) {
          let history: Array<{ role: string; content: string; ts?: number }> = [];
          try { history = JSON.parse(row.messageHistory ?? '[]'); } catch { continue; }
          if (history.length === 0) continue;

          const lastReadAt = (row.lastReadAt as number | null) ?? 0;
          let unreadCount = 0;
          let lastCustomerMsgTs: number | null = null;
          for (const m of history) {
            if ((m.role === 'user' || m.role === 'customer') && m.ts) {
              if (m.ts > lastReadAt) unreadCount++;
              if (!lastCustomerMsgTs || m.ts > lastCustomerMsgTs) lastCustomerMsgTs = m.ts;
            }
          }

          if (unreadCount > 0 && row.stage !== 'RESOLVED') {
            results.push({
              sessionId: row.id,
              phone: row.leadPhone,
              leadName: row.leadName,
              stage: row.stage,
              lastReadAt: row.lastReadAt as number | null,
              lastCustomerReplyAt: row.lastCustomerReplyAt as number | null,
