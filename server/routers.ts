    ...
    getLeadReplies: opsChatProcedure.query(async () => {
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            // Exact same sourceFilter as leads.list — ensures counts match Unreads on leads page
            sql`(
              ${conversationSessions.leadSource} IS NULL OR
              ${conversationSessions.leadSource} NOT IN (${sql.raw(NON_LEAD_SOURCES.map(s => `'${s}'`).join(', '))}) AND
              NOT (
      ...
      for (const r of rows) {
        try {
          ...
          // Only include unread leads — matches Unreads filter on leads page exactly
