...
describe("Leads CRM exact-live shell", () => {
  it("uses the approved Leads CRM composition instead of the prior leads dashboard shell", () => {
    expect(page).toContain('className="operations-crm-review leads-crm-live"');
    expect(page).toContain('className="ocr-sidebar"');
  ...
  it("renders actual incoming lead fields and does not retain review sample data", () => {
    ...
    expect(page).toContain("Quoted amount");
    expect(page).toContain("Booked amount");
    expect(page).toContain("Actual message activity over the last thirty days");
  ...
  it("gates the default admin leads route while preserving existing utility deep links", () => {
    ...
    expect(route).toContain('return <AdminPageGuard pageId="leads"><LeadsCRMExactLive /></AdminPageGuard>');
