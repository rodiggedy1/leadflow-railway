...
import { completeCustomerPortalServiceBooking } from "./customerPortalServiceBookingCompletion";
...
export const customerPortalRouter = router({
  ...
  createRequest: publicProcedure.input(requestSchema).mutation(async ({ ctx, input }) => {
    ...
    return completeCustomerPortalServiceBooking({
      account,
      input,
      service,
      estimate: calculateCustomerPortalEstimate(service.id, input.selections),
      savedCard,
    });
  }),
});
