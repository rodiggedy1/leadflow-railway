...
describe("Command Chat Email popup detail", () => {
  it("uses the CsInbox2 direct-detail treatment when Command Chat supplies a selected thread", () => {
    ...
    expect(source).toContain('DOMPurify.sanitize(message.bodyHtml, { USE_PROFILES: { html: true }, FORBID_ATTR: ["style", "color", "bgcolor"] })');
    ...
    expect(source).toContain('const popupThreadMessages = popupPrimaryMessage');
    expect(source).toContain('showClose && popupPrimaryMessage && <>');
